...
:regtitle: pass:q,r[^®^]
Editor: Radhakrishna Giduthuri; The Khronos{regtitle} OpenVX Working Group
...
The hierarchy of such vision function namespaces is undefined outside the
...
This specification defines the following OpenVX framework objects.
  ...
  * <<group_context>> - The OpenVX context is the object domain for all
    ...
    different contexts are undefined.
  ...
  * <<group_parameter>> - An abstract input, output, or bidirectional data
  ...
  ** _Usage Model_ - e.g. `<<VX_INPUT,VX_INPUT>>`, `<<VX_OUTPUT,VX_OUTPUT>>`, or
     `<<VX_BIDIRECTIONAL,VX_BIDIRECTIONAL>>`.
...
different in the following respects.
  ...
  * Scoped - Virtual data objects are scoped within the Graph in which they
    ...
    In other word, data content of a virtual object is undefined before
  ...
  * Dimensionless or Formatless - Virtual data objects may have dimensions
    ...
    undefined with regard to the graph and mutable at graph verification
...
In addition, the Processing Graph has several restrictions:
  ...
  . _Output typing_ - Every output edge [eq]#(N~x~,D~y~)# requires
    [eq]#`Type`(N~x~, `Name`(N~x~,D~y~))# in `{OUTPUT, INOUT}`
  . _Input typing_ - Every input edge [eq]#(N~x~,D~y~)# requires
    [eq]#`Type`(N~y~, `Name`(D~x~,N~y~))# in `{INPUT}` or `{INOUT}`
  ...
  . _Bidirectional data objects shall not be virtual_ - If [eq]#`Type`(N~x~,
    `Name`(N~x~,D~y~))# is `INOUT` implies [eq]#D~y~# is non-Virtual.
  . _Delay data objects shall not be virtual_ - If [eq]#D~x~# is Delay then
    it shall not be Virtual.
  . _A uniform image cannot be output or bidirectional_.
...
Formalization of the valid region mechanism is given below.
  ...
  * Valid Rectangle Calculation - The valid rectangle of an image changes as
    ...
    size of the valid region are implementation-dependent.
...
See <<group_hint>>.
It is assumed that most of the hints will be vendor- or
...
cache writing-through for some buffers, or any implementation-specific
optimization.
...
|===
| <<group_vision_function_absdiff,Absolute Difference>> | <<group_vision_function_accumulate,Accumulate>> | <<group_vision_function_accumulate_square,Accumulate Squared>>
| <<group_vision_function_accumulate_weighted,Accumulate Weighted>> | <<group_vision_function_add,Arithmetic Addition>> | <<group_vision_function_sub,Arithmetic Subtraction>>
...
`<<VX_CONVERT_POLICY_SATURATE,VX_CONVERT_POLICY_SATURATE>>` is used.
...
*Parameters*
  ...
  * `[out]` _out_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format, which must have the same dimensions as the
    input image.
...
[[group_vision_function_accumulate]]
== Accumulate
...
The accumulation image dimensions should be the same as the dimensions of
the input image.

// @details
Accumulation is computed by:

  :: [eq]#accum(x,y) = accum(x,y) + input(x,y)#

The overflow policy used is `<<VX_CONVERT_POLICY_SATURATE,VX_CONVERT_POLICY_SATURATE>>`.

// tag::group_vision_function_accumulate:summary[]
*Functions*

  * `<<vxAccumulateImageNode>>`
  * `<<vxuAccumulateImage>>`

// end::group_vision_function_accumulate:summary[]

// tag::group_vision_function_accumulate:details[]
=== Functions

// tag::refpage:vxAccumulateImageNode[]
// [desc='',type='protos']
==== vxAccumulateImageNode
...
include::api/protos/vxAccumulateImageNode.txt[]
...
*Parameters*
  ...
  * `[inout]` _accum_ - The accumulation image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`,
...
*Return Values*
  ...
  * `<<vx_node>>` - A node reference.
    Any possible errors preventing a successful creation should be checked
    using `<<vxGetStatus>>`
// end::refpage:vxAccumulateImageNode[]

// tag::refpage:vxuAccumulateImage[]
// [desc='',type='protos']
==== vxuAccumulateImage
...
include::api/protos/vxuAccumulateImage.txt[]
...
*Return Values*
  ...
  * * - An error occurred.
    See `<<vx_status_e>>`.
// end::refpage:vxuAccumulateImage[]

// end::group_vision_function_accumulate:details[]



[[group_vision_function_accumulate_square]]
== Accumulate Squared
...
Accumulate squares is computed by:
  ...
  :: [eq]#accum(x,y) = saturate~int16~ ( (uint16) accum(x,y) {plus}
     ( ( (uint16)(input(x,y)^2^)) >> (shift)))#

Where [eq]#0 {leq} shift {leq} 15#

The overflow policy used is `<<VX_CONVERT_POLICY_SATURATE,VX_CONVERT_POLICY_SATURATE>>`.

// tag::group_vision_function_accumulate_square:summary[]
*Functions*

  * `<<vxAccumulateSquareImageNode>>`
  * `<<vxuAccumulateSquareImage>>`

// end::group_vision_function_accumulate_square:summary[]

// tag::group_vision_function_accumulate_square:details[]
=== Functions

// tag::refpage:vxAccumulateSquareImageNode[]
// [desc='',type='protos']
==== vxAccumulateSquareImageNode
...
include::api/protos/vxAccumulateSquareImageNode.txt[]
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph.
  * `[in]` _input_ - The input `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` image.
  * `[in]` _shift_ - The input `<<VX_TYPE_UINT32,VX_TYPE_UINT32>>` with a value in the
    range of [eq]#0 {leq} shift {leq} 15#.
  * `[inout]` _accum_ - The accumulation image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`,
    which must have the same dimensions as the input image.
...
*Return Values*
  ...
  * `<<vx_node>>` - A node reference.
    Any possible errors preventing a successful creation should be checked
    using `<<vxGetStatus>>`
// end::refpage:vxAccumulateSquareImageNode[]

// tag::refpage:vxuAccumulateSquareImage[]
// [desc='',type='protos']
==== vxuAccumulateSquareImage
...
include::api/protos/vxuAccumulateSquareImage.txt[]
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _input_ - The input `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` image.
  * `[in]` _shift_ - A `<<VX_TYPE_UINT32,VX_TYPE_UINT32>>` type, the input value with the
    range [eq]#0 {leq} shift {leq} 15#.
  * `[inout]` _accum_ - The accumulation image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`
...
*Return Values*
  ...
  * * - An error occurred.
    See `<<vx_status_e>>`.
// end::refpage:vxuAccumulateSquareImage[]

// end::group_vision_function_accumulate_square:details[]



[[group_vision_function_accumulate_weighted]]
== Accumulate Weighted
...
Weighted accumulation is computed by:
  ...
  :: [eq]#accum(x,y) = (1 - {alpha}) accum(x,y) + {alpha} input(x,y)#

Where [eq]#0 {leq} {alpha} {leq} 1#.
Conceptually, the rounding for this is defined as:

  :: [eq]#output(x,y)= uint8( (1 - {alpha}) float32( int32( output(x,y) ) )
     + {alpha} float32( int32( input(x,y) ) ) )#

// tag::group_vision_function_accumulate_weighted:summary[]
*Functions*

  * `<<vxAccumulateWeightedImageNode>>`
  * `<<vxuAccumulateWeightedImage>>`

// end::group_vision_function_accumulate_weighted:summary[]

// tag::group_vision_function_accumulate_weighted:details[]
=== Functions

// tag::refpage:vxAccumulateWeightedImageNode[]
// [desc='',type='protos']
==== vxAccumulateWeightedImageNode
...
include::api/protos/vxAccumulateWeightedImageNode.txt[]
...
*Parameters*
  ...
  * `[in]` _alpha_ - The input `<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` scalar value with a
    value in the range of [eq]#0.0 {leq} {alpha} {leq} 1.0#.
  * `[inout]` _accum_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` accumulation image, which
...
*Return Values*
  ...
  * `<<vx_node>>` - A node reference.
    Any possible errors preventing a successful creation should be checked
    using `<<vxGetStatus>>`
// end::refpage:vxAccumulateWeightedImageNode[]

// tag::refpage:vxuAccumulateWeightedImage[]
// [desc='',type='protos']
==== vxuAccumulateWeightedImage
...
include::api/protos/vxuAccumulateWeightedImage.txt[]
...
*Parameters*
  ...
  * `[inout]` _accum_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` accumulation image.
...
*Return Values*
  ...
  * * - An error occurred.
    See `<<vx_status_e>>`.
// end::refpage:vxuAccumulateWeightedImage[]

// end::group_vision_function_accumulate_weighted:details[]
...
`<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` images.
The output image can be `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` only if both source images are
`<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` and the output image is explicitly set to
`<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>`.
It is otherwise `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`.
...
*Parameters*
  ...
  * `[out]` _out_ - The output image, a `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` image, which must have the same dimensions as the
    input images.
...
`<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or two `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` images.
The output image can be `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` only if both source images are
`<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` and the output image is explicitly set to
`<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>`.
It is otherwise `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph.
  * `[in]` _in1_ - An input image, `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`, the minuend.
  * `[in]` _in2_ - An input image, `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`, the subtrahend.
  * `[in]` _policy_ - A `<<VX_TYPE_ENUM,VX_TYPE_ENUM>>` of the `<<vx_convert_policy_e>>`
    enumeration.
  * `[out]` _out_ - The output image, a `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` image, which must have the same dimensions as the
...
*Parameters*
  ...
  * `[in]` _src_ - The input data, a `<<vx_tensor>>`.
    ...
    Dimensions are [radiometric,width,height] or [width,height].See
...
// @brief
Performs a _bitwise AND_ operation between two `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` images.
The output image dimensions should be the same as the dimensions of the
input images.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph.
  * `[in]` _in1_ - A `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` input image.
  * `[in]` _in2_ - A `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` input image.
  * `[out]` _out_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` output image, which must have
    the same dimensions as the input images.
...
*Parameters*
  ...
  * `[in]` _in1_ - A `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` input image
  * `[in]` _in2_ - A `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` input image
  * `[out]` _out_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` output image.
...
`<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` images.
The output image dimensions should be the same as the dimensions of the
input images.
...
// @brief
Performs a _bitwise INCLUSIVE OR_ operation between two `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>`
images.
The output image dimensions should be the same as the dimensions of the
input images.
...
// @brief
Performs a _bitwise NOT_ operation on a `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` input image.
The output image dimensions should be the same as the dimensions of the
input image.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph.
  * `[in]` _input_ - A `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` input image.
  * `[out]` _output_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` output image, which must
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _input_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` input image
  * `[out]` _output_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` output image.
...
known edge pixels (valued 255) in the final output image.
...
Only the final binary (0 or 255 valued) output image must be computed so
that it matches the result of a final image constructed as described above.
...
*Parameters*
  ...
  * `[out]` _output_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` format with
...
*Parameters*
  ...
  * `[in]` _graph_ - The graph reference.
...
*Parameters*
  ...
  * `[in]` _channel_ - The `<<vx_channel_e>>` channel to extract.
  * `[out]` _output_ - The output image.
    Must be `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>`, and must have the same dimensions as the
    input image.
...
`<<vx_channel_range_e>>`) attributes of the image.
Implementations are required only to support images of
`<<VX_COLOR_SPACE_BT709,VX_COLOR_SPACE_BT709>>` and `<<VX_CHANNEL_RANGE_FULL,VX_CHANNEL_RANGE_FULL>>`.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph.
  * `[in]` _input_ - The input image from which to convert.
...
`<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` are supported.
...
Please note that in the above tables:
  ...
  * The `<<VX_SCALAR_OP_MODULUS,VX_SCALAR_OP_MODULUS>>` operation supports integer operands.
  ...
  * If the operation result can not be stored in output data type without
  ...
  .. If the operation result is integer and output is floating-point, the
     operation result is promoted to floating-point.
  .. If the operation result is floating-point and output is an integer, the
     ...
     `<<VX_CONVERT_POLICY_SATURATE,VX_CONVERT_POLICY_SATURATE>>`.
...
*Parameters*
  ...
  * `[in]` _scalar_operation_ - A `<<VX_TYPE_ENUM,VX_TYPE_ENUM>>` of the
    `<<vx_scalar_operation_e>>` enumeration.
  * `[in]` _a_ - First scalar operand.
  * `[in]` _b_ - Second scalar operand.
  * `[out]` _output_ - Result of the scalar operation.
...
condition (boolean scalar), and copies its data into another data object.
...
that there will not be any side effects to graph execution and node state.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph.
  * `[in]` _condition_ - `<<VX_TYPE_BOOL,VX_TYPE_BOOL>>` predicate variable.
  * `[in]` _true_value_ - Data object for true.
  * `[in]` _false_value_ - Data object for false.
  * `[out]` _output_ - Output data object.
...
bit-depth as described by the table below.
If the input value is unsigned the shift must be in zeros.
If the input value is signed, the shift used must be an arithmetic shift.
...
Down-conversions with `<<VX_CONVERT_POLICY_SATURATE,VX_CONVERT_POLICY_SATURATE>>` follow this equation:
...
The valid values for 'shift' are as specified below, all other values
...
`<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` image is explicitly provided.
...
For `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` output, an additional step is taken:
...
// @details
Copy data from an input data object into another data object.
...
*Parameters*
  ...
  * `[out]` _output_ - The grayscale output image of type
    `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` with equalized brightness and contrast and same
...
`<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` Boolean image.
The output image dimensions should be the same as the dimensions of the
input image.
...
value of strength is undefined.
...
*Parameters*
  ...
  * `[in]` _nonmax_suppression_ - If true, non-maximum suppression is
    ...
    `<<VX_TYPE_KEYPOINT,VX_TYPE_KEYPOINT>>` objects.
...
image to the next level using `<<VX_INTERPOLATION_NEAREST_NEIGHBOR,VX_INTERPOLATION_NEAREST_NEIGHBOR>>`.
...
Each position produces a HOG feature vector.
...
that satisfy: [eq]#0 {leq} max_dist {leq} 30# The feature set [eq]#F# is
...
*Parameters*
  ...
  * `[in]` _block_size_ - The block window size used to compute the Harris
    Corner score.
    The implementation must support at least 3, 5, and 7.
  * `[out]` _corners_ - The array of `<<VX_TYPE_KEYPOINT,VX_TYPE_KEYPOINT>>` objects.
    The order of the keypoints in this array is implementation dependent.
  * `[out]` _num_corners_ - [optional] The total number of detected corners
    in image.
    Use a `<<VX_TYPE_SIZE,VX_TYPE_SIZE>>` scalar.
...
Using the Progressive Probabilistic Hough Transform.
...
*Parameters*
  ...
  * `[in]` _graph_ - graph handle
  * `[in]` _input_ - 8 bit, single channel binary source image
  * `[in]` _params_ - parameters of the struct `<<vx_hough_lines_p_t>>`
  * `[out]` _lines_array_ - lines_array contains array of lines, see
    `<<vx_line2d_t>>` The order of lines in implementation dependent
...
pixels above and to the left of it.
...
The overflow policy used is `<<VX_CONVERT_POLICY_WRAP,VX_CONVERT_POLICY_WRAP>>`.
...
Pattern, Modified Local Binary Pattern, or Uniform Local Binary Pattern.
...
The LBP image is defined in the following equation:
...
pixels with its 8 neighbours (5x5 window).
...
[eq]#L~0~# shall always have the same resolution as the input image.
...
*Parameters*
  ...
  * `[out]` _mag_ - The magnitude image.
    This is in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format.
    Must have the same dimensions as the input image.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph where to create the node.
  ...
  * `[out]` _out_ - The output image which will hold the result of max and
    will have the same type and dimensions of the imput images.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph.
  * `[in]` _input_ - The input image.
    `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` is supported.
...
*Parameters*
  ...
  * `[in]` _input_ - The input image.
    `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` is supported.
  * `[out]` _mean_ - The average pixel value.
  * `[out]` _stddev_ - [optional] The standard deviation of the pixel
    values.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to create the graph.
  * `[in]` _input_ - The input image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format.
...
suppressed and not considered as part of any suppression window.
...
*Parameters*
  ...
  * `[in]` _mask_ - [optional] Constrict suppression to a ROI.
    The mask image is of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` and must be the same
  ...
  * `[out]` _output_ - The output image, of the same type and size as the
    input, that has been non-maxima suppressed.
...
`<<VX_SCALE_PYRAMID_HALF,VX_SCALE_PYRAMID_HALF>>` pyramidal scaling must be supported.

The function outputs a `<<vx_array>>` of `<<vx_keypoint_t>>` structs that
were tracked from the old `<<vx_pyramid>>` to the new `<<vx_pyramid>>`.
...
structs in the new `<<vx_array>>` that were in the older `<<vx_array>>`.
...
_tracking_status_ set to zero (lost) are copied to the new `<<vx_array>>`.
...
the old tracking `<<vx_keypoint_t>>` to new image `<<vx_keypoint_t>>`.
...
*Parameters*
  ...
  * `[in]` _old_points_ - An array of key points in a `<<vx_array>>` of
    `<<VX_TYPE_KEYPOINT,VX_TYPE_KEYPOINT>>`; those key points are defined at the _old_images_
  ...
  * `[out]` _new_points_ - An output array of key points in a `<<vx_array>>`
    of `<<VX_TYPE_KEYPOINT,VX_TYPE_KEYPOINT>>`; those key points are defined at the
  ...
  * `[in]` _use_initial_estimate_ - Use a `<<VX_TYPE_BOOL,VX_TYPE_BOOL>>` scalar.
...
Each [eq]#{phi}# value is then mapped to the range 0 to 255 inclusive.
...
*Parameters*
  ...
  * `[out]` _orientation_ - The phase image.
    This is in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` format, and must have the same
...
`<<VX_ROUND_POLICY_TO_NEAREST_EVEN,VX_ROUND_POLICY_TO_NEAREST_EVEN>>` must be supported whereas for the
scale with value of latexmath:[\frac{1}{2^n}] the rounding policy of
`<<VX_ROUND_POLICY_TO_ZERO,VX_ROUND_POLICY_TO_ZERO>>` must be supported.
...
*Parameters*
  ...
  * `[out]` _out_ - The output image, a `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` image.
    Must have the same type and dimensions of the imput images.
...
resolution from a Laplacian pyramid.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph that will contain the node.
  * `[in]` _input_ - The input `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` image.
...
The supported interpolation types are currently:
  ...
  * `<<VX_INTERPOLATION_NEAREST_NEIGHBOR,VX_INTERPOLATION_NEAREST_NEIGHBOR>>`
  * `<<VX_INTERPOLATION_AREA,VX_INTERPOLATION_AREA>>`
  * `<<VX_INTERPOLATION_BILINEAR,VX_INTERPOLATION_BILINEAR>>`
...
defined by the following equations:
  ...
  * For `<<VX_INTERPOLATION_NEAREST_NEIGHBOR,VX_INTERPOLATION_NEAREST_NEIGHBOR>>`, the output value is that of
    the pixel whose centre is closest to the sample point.
  * For `<<VX_INTERPOLATION_BILINEAR,VX_INTERPOLATION_BILINEAR>>`, the output value is formed by a
    weighted average of the nearest source pixels to the sample point.
    That is:
+
--
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph.
  * `[in]` _src_ - The source image of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>`.
  * `[out]` _dst_ - The destination image of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>`.
...
`<<VX_BORDER_REPLICATE,VX_BORDER_REPLICATE>>` and `<<VX_BORDER_CONSTANT,VX_BORDER_CONSTANT>>` are supported.
====
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _src_ - The source image of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>`.
  * `[out]` _dst_ - The destintation image of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>`.
...
plane.
The Sobel Operators [eq]#G~x~, G~y~# are defined as:
...
The formats supported are `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` and `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`.
...
The equation for the conversion is as follows:
...
// @details
Pixel-wise multiplication is performed between the pixel values in two
tensors and a scalar floating-point number _scale_.
...
The tensor types supported are `<<VX_TYPE_UINT8,VX_TYPE_UINT8>>` and `<<VX_TYPE_INT16,VX_TYPE_INT16>>`.
...
// @details
In `<<VX_THRESHOLD_TYPE_BINARY,VX_THRESHOLD_TYPE_BINARY>>`, the output is determined by:
...
In `<<VX_THRESHOLD_TYPE_RANGE,VX_THRESHOLD_TYPE_RANGE>>`, the output is determined by:
...
*Parameters*
  ...
  * `[out]` _output_ - The output image, that will contain as pixel value
    ...
    Only images with format `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` are supported.
    The dimensions are the same as the input image.
...
method of pixel coordinate translation:
...
*Parameters*
  ...
  * `[in]` _type_ - The interpolation type from
    `<<vx_interpolation_type_e>>`. `<<VX_INTERPOLATION_AREA,VX_INTERPOLATION_AREA>>` is not
    supported.
  * `[out]` _output_ - The output `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` image and the same
    dimensions as the input image.
...
this method of pixel coordinate translation:
...
*Parameters*
  ...
  * `[in]` _reference_ - The reference to check for construction errors.
...
*Enumerator*
  ...
  * [[VX_READ_ONLY]] `VX_READ_ONLY` - The memory shall be treated by the
    ...
    If the User writes to this memory, the results are implementation
    defined.
  * [[VX_WRITE_ONLY]] `VX_WRITE_ONLY` - The memory shall be treated by the
    system as if it were write-only.
    If the User reads from this memory, the results are implementation
    defined.
...
*Enumerator*
  ...
  * [[VX_CONTEXT_UNIQUE_KERNELS]] `VX_CONTEXT_UNIQUE_KERNELS` - Queries the
    context for the number of _unique_ kernels.
    Read-only.
    Use a `<<vx_uint32>>` parameter.
  ...
  * [[VX_CONTEXT_REFERENCES]] `VX_CONTEXT_REFERENCES` - Queries the context
    for the number of active references.
    Read-only.
    Use a `<<vx_uint32>>` parameter.
  * [[VX_CONTEXT_IMPLEMENTATION]] `VX_CONTEXT_IMPLEMENTATION` - Queries the
    context for it's implementation name.
    Read-only.
  ...
  * [[VX_CONTEXT_EXTENSIONS_SIZE]] `VX_CONTEXT_EXTENSIONS_SIZE` - Queries
    the number of bytes in the extensions string.
    Read-only.
    Use a `<<vx_size>>` parameter.
  * [[VX_CONTEXT_EXTENSIONS]] `VX_CONTEXT_EXTENSIONS` - Retrieves the
    extensions string.
    Read-only.
    This is a space-separated string of extension names.
    ...
    "KHR_", for example "KHR_NEW_FEATURE".
  ...
  * [[VX_CONTEXT_CONVOLUTION_MAX_DIMENSION]]
    ...
    supported.
    The behavior of `<<vxCreateConvolution>>` is undefined for values larger
  ...
  * [[VX_CONTEXT_OPTICAL_FLOW_MAX_WINDOW_DIMENSION]]
    ...
    dimension of the <<OpticalFlowPyrLK>> kernel.
    The value of this attribute shall be equal to or greater than '9'.
+
--
// Detailed text
...
--
  * [[VX_CONTEXT_UNIQUE_KERNEL_TABLE]] `VX_CONTEXT_UNIQUE_KERNEL_TABLE` -
    Returns the table of all unique the kernels that exist in the context.
    Read-only.
...
--
  * [[VX_CONTEXT_MAX_TENSOR_DIMS]] `VX_CONTEXT_MAX_TENSOR_DIMS` - tensor
    Data maximal number of dimensions supported by the implementation.
...
*Enumerator*
  ...
  * [[VX_ROUND_POLICY_TO_ZERO]] `VX_ROUND_POLICY_TO_ZERO` - When scaling,
    this truncates the least significant values that are lost in operations.
...
*Enumerator*
  ...
  * [[VX_TERM_CRITERIA_BOTH]] `VX_TERM_CRITERIA_BOTH` - Indicates that both
    an iterations and eplison method are employed.
    Whichever one matches first causes the termination.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the context.
...
*Parameters*
  ...
  * `[in]` _context_ - The pointer to the reference to the context.
...
*Parameters*
  ...
  * `[in]` _context_ - The handle to the overall context.
...
*Returns:* A graph reference `<<vx_graph>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph to check.

*Returns:* A `<<vx_bool>>` value.

*Return Values*

  * `<<vx_true_e,vx_true_e>>` - The graph is verified.
  * `<<vx_false_e,vx_false_e>>` - The graph is not verified.
...
`<<vxVerifyGraph>>` would return.
After the graph verfies successfully then processing occurs.
If the graph was previously verified via `<<vxVerifyGraph>>` or
`<<vxProcessGraph>>` then the graph is processed.
This function blocks until the graph is completed.
...
*Parameters*
  ...
  * `[in]` _graph_ - The graph to execute.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the created graph.
  * `[in]` _attribute_ - The `<<vx_graph_attribute_e>>` type needed.
  * `[out]` _ptr_ - The location at which to store the resulting value.
  * `[in]` _size_ - The size in bytes of the container to which _ptr_
    points.
...
delay will be only aged a single time at each graph completion.
...
*Parameters*
  ...
  * `[in]` _graph_ - The graph to which the delay is registered for
    auto-aging.
  * `[in]` _delay_ - The delay to automatically age.
...
*Parameters*
  ...
  * `[in]` _graph_ - The pointer to the graph to release.
...
*Parameters*
  ...
  * `[in]` _graph_ - The graph to schedule.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph.
  * `[in]` _attribute_ - The `<<vx_graph_attribute_e>>` type needed.
  * `[in]` _ptr_ - The location from which to read the value.
  * `[in]` _size_ - The size in bytes of the container to which _ptr_
    points.
...
// Brief text:
Verifies the state of the graph before it is executed.
This is useful to catch programmer errors and contract errors.
If not verified, the graph verifies before being processed.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph to verify.

*Returns:* A status code for graphs with more than one error; it is
undefined which error will be returned.
...
*Parameters*
  ...
  * `[in]` _graph_ - The graph to wait on.
...
*Return Values*
  ...
  * `<<VX_SUCCESS,VX_SUCCESS>>` - The graph has successfully completed execution and
    its outputs are the valid results of the most recent execution; any
    other value indicates failure.
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - graph is not a valid `<<vx_graph>>`
    reference.
  * `<<VX_FAILURE,VX_FAILURE>>` - An error occurred or the graph was never scheduled.
    Output data of the graph is undefined.
...
--
  ...
  * [[VX_NODE_LOCAL_DATA_SIZE]] `VX_NODE_LOCAL_DATA_SIZE` - Indicates the
    size of the kernel local memory area.
    Read-only.
  ...
  * [[VX_NODE_VALID_RECT_RESET]] `VX_NODE_VALID_RECT_RESET` - Indicates the
    behavior with respect to the valid rectangle.
    Read-only.
    Use a `<<vx_bool>>` parameter.
...
*Parameters*
  ...
  * `[in]` _node_ - The reference to the node to query.
...
// Brief text:
Releases a reference to a Node object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _node_ - The pointer to the reference of the node to release.
...
*Parameters*
  ...
  * `[in]` _node_ - The pointer to the node to remove and release.
...
*Parameters*
  ...
  * `[in]` _replicate_ - an array of size equal to the number of node
    parameters, `<<vx_true_e,vx_true_e>>` for the parameters that should be iterated
    ...
    not used.
    Should be `<<vx_true_e,vx_true_e>>` for all output and bidirectional parameters.
...
*Parameters*
  ...
  * `[in]` _node_ - The reference to the node to set.
  * `[in]` _attribute_ - Use `<<vx_node_attribute_e>>` value to set the
    desired attribute.
  * `[in]` _ptr_ - The pointer to the desired value of the attribute.
  * `[in]` _size_ - The size in bytes of the objects to which _ptr_ points.
...
// Brief text:
Sets the node target to the provided value.
...
*Parameters*
  ...
  * `[in]` _node_ - The reference to the `<<vx_node>>` object.
...
*Return Values*
  ...
  * `<<VX_SUCCESS,VX_SUCCESS>>` - Node target set; any other value indicates failure.
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - node is not a valid `<<vx_node>>`
    reference.
...
// Brief text:
The Array Object.
Array is a strongly-typed container for other data structures.
...
*Enumerator*
  ...
  * [[VX_ARRAY_ITEMTYPE]] `VX_ARRAY_ITEMTYPE` - The type of the Array items.
    Read-only.
    Use a `<<vx_enum>>` parameter.
  ...
  * [[VX_ARRAY_CAPACITY]] `VX_ARRAY_CAPACITY` - The maximal number of items
    ...
    Read-only.
    Use a `<<vx_size>>` parameter.
  * [[VX_ARRAY_ITEMSIZE]] `VX_ARRAY_ITEMSIZE` - Queries an array item size.
    Read-only.
    Use a `<<vx_size>>` parameter.
...
*Parameters*
  ...
  * `[in]` _arr_ - The reference to the Array.
  * `[in]` _count_ - The total number of elements to insert.
  * `[in]` _ptr_ - The location from which to read the input values.
  * `[in]` _stride_ - The number of bytes between the beginning of two
    consecutive elements.
...
*Parameters*
  ...
  * `[in]` _user_stride_ - The number of bytes between the beginning of two
    consecutive items in the user memory pointed by user_ptr.
  ...
  * `[in]` _usage_ - This declares the effect of the copy with regard to the
    array object using the `<<vx_accessor_e>>` enumeration.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall Context.
  * `[in]` _item_type_ - The type of data to hold.
...
*Returns:* An array reference `<<vx_array>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Returns:* A array reference `<<vx_array>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[out]` _stride_ - The address of a `<<vx_size>>` variable where the
    function returns the memory layout of the mapped array range.
  ...
  ** `<<VX_READ_AND_WRITE,VX_READ_AND_WRITE>>`: after the function call, the content of the
     ...
     Writing into a gap between items (when (*stride) > item size in bytes)
     is forbidden and its behavior is undefined.
  ** `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>`: after the function call, the memory location
     pointed by (*_ptr_) contains undefined data; writing each item of the
     range is required prior to unmapping.
     Items not written by the application before unmap will become undefined
     after unmap, even if they were well defined before map.
     Like for `<<VX_READ_AND_WRITE,VX_READ_AND_WRITE>>`, writing into a gap between items is
     forbidden and its behavior is undefined.
  * `[in]` _mem_type_ - A `<<vx_memory_type_e>>` enumeration that specifies
    the type of the memory where the array range is requested to be mapped.
...
// Brief text:
Releases a reference of an Array object.
The object may not be garbage collected until its total reference count is
zero.
After returning from this function the reference is zeroed.
...
*Parameters*
  ...
  * `[in]` _arr_ - The pointer to the Array to release.
...
*Parameters*
  ...
  * `[inout]` _arr_ - The reference to the Array.
  * `[in]` _new_num_items_ - The new number of items for the Array.
...
*Parameters*
  ...
  * `[in]` _array_ - The reference to the array object to unmap.
  * `[out]` _map_id_ - The unique map identifier that was returned when
    calling `<<vxMapArrayRange>>`.
...
A user-defined convolution kernel of MxM elements.
...
*Enumerator*
  ...
  * [[VX_CONVOLUTION_COLUMNS]] `VX_CONVOLUTION_COLUMNS` - The number of
    columns of the convolution matrix.
    Read-only.
    Use a `<<vx_size>>` parameter.
...
*Parameters*
  ...
  * `[in]` _conv_ - The reference to the convolution object that is the
    source or the destination of the copy.
  * `[in]` _user_ptr_ - The address of the memory location where to store
    the requested coefficient data if the copy was requested in read mode,
  ...
  * `[in]` _usage_ - This declares the effect of the copy with regard to the
    convolution object using the `<<vx_accessor_e>>` enumeration.
    Only `<<VX_READ_ONLY,VX_READ_ONLY>>` and `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>` are supported:
  ** `<<VX_READ_ONLY,VX_READ_ONLY>>` means that data are copied from the convolution
     object into the user memory.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _columns_ - The columns dimension of the convolution.
    Must be odd and greater than or equal to 3 and less than the value
    returned from `<<VX_CONTEXT_CONVOLUTION_MAX_DIMENSION,VX_CONTEXT_CONVOLUTION_MAX_DIMENSION>>`.
  * `[in]` _rows_ - The rows dimension of the convolution.
    Must be odd and greater than or equal to 3 and less than the value
    returned from `<<VX_CONTEXT_CONVOLUTION_MAX_DIMENSION,VX_CONTEXT_CONVOLUTION_MAX_DIMENSION>>`.

*Returns:* A convolution reference `<<vx_convolution>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _conv_ - The convolution matrix object to set.
  * `[in]` _attribute_ - The attribute to query.
...
// Brief text:
Releases the reference to a convolution matrix.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _conv_ - The pointer to the convolution matrix to release.
...
*Parameters*
  ...
  * `[in]` _conv_ - The coordinates object to set.
  * `[in]` _attribute_ - The attribute to modify.
    Use a `<<vx_convolution_attribute_e>>` enumeration.
  * `[in]` _ptr_ - The pointer to the value to which to set the attribute.
  * `[in]` _size_ - The size in bytes of the data pointed to by _ptr_.
...
*Enumerator*
  ...
  * [[VX_DISTRIBUTION_OFFSET]] `VX_DISTRIBUTION_OFFSET` - Indicates the
    start of the values to use (inclusive).
    Read-only.
    Use a `<<vx_int32>>` parameter.
  * [[VX_DISTRIBUTION_RANGE]] `VX_DISTRIBUTION_RANGE` - Indicates the total
    number of the consecutive values of the distribution interval.
    Read-only.
    Use a `<<vx_uint32>>` parameter.
...
*Parameters*
  ...
  * `[in]` _usage_ - This declares the effect of the copy with regard to the
    distribution object using the `<<vx_accessor_e>>` enumeration.
    Only `<<VX_READ_ONLY,VX_READ_ONLY>>` and `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>` are supported:
...
divided equally into _numBins_ parts.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _numBins_ - The number of bins in the distribution.
  * `[in]` _offset_ - The start offset into the range value that marks the
    begining of the 1D Distribution.
  * `[in]` _range_ - The total number of the consecutive values of the
    distribution interval.

*Returns:* A distribution reference `<<vx_distribution>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _distribution_ - The reference to the distribution object to map.
  * `[out]` _map_id_ - The address of a `<<vx_map_id>>` variable where the
    function returns a map identifier.
  ...
  * `[out]` _ptr_ - The address of a pointer that the function sets to the
    ...
    with a range-major ordering.
    Accessing the memory out of the bound of this array is forbidden and has
    an undefined behavior.
    The returned (*_ptr_) address is only valid between the call to the
    function and the corresponding call to `<<vxUnmapDistribution>>`.
  * `[in]` _usage_ - This declares the access mode for the distribution,
    using the `<<vx_accessor_e>>` enumeration.
  ** `<<VX_READ_ONLY,VX_READ_ONLY>>`: after the function call, the content of the memory
     location pointed by (*_ptr_) contains the distribution data.
     Writing into this memory location is forbidden and its behavior is
     undefined.
  ** `<<VX_READ_AND_WRITE,VX_READ_AND_WRITE>>`: after the function call, the content of the
     memory location pointed by (*_ptr_) contains the distribution data;
  ...
  ** `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>`: after the function call, the memory location
     pointed by (*_ptr_) contains undefined data; writing each bin of
     distribution is required prior to unmapping.
     Bins not written by the application before unmap will become undefined
     after unmap, even if they were well defined before map.
  * `[in]` _mem_type_ - A `<<vx_memory_type_e>>` enumeration that specifies
    the type of the memory where the distribution is requested to be mapped.
  * `[in]` _flags_ - An integer that allows passing options to the map
    operation.
...
*Parameters*
  ...
  * `[in]` _distribution_ - The reference to the distribution to query.
  * `[in]` _attribute_ - The attribute to query.
...
*Parameters*
  ...
  * `[in]` _distribution_ - The reference to the distribution to release.
...
*Enumerator*
  ...
  * [[VX_IMAGE_WIDTH]] `VX_IMAGE_WIDTH` - Queries an image for its width.
    Read-only.
    Use a `<<vx_uint32>>` parameter.
  * [[VX_IMAGE_HEIGHT]] `VX_IMAGE_HEIGHT` - Queries an image for its height.
    Read-only.
    Use a `<<vx_uint32>>` parameter.
  * [[VX_IMAGE_FORMAT]] `VX_IMAGE_FORMAT` - Queries an image for its format.
    Read-only.
...
*Parameters*
  ...
  * `[in]` _user_addr_ - The address of a structure describing the layout of
    ...
    {geq} pixel size in bytes, and _stride_y_ {geq} _stride_x_ * _dim_x_.
  * `[in]` _user_ptr_ - The address of the memory location where to store
    the requested data if the copy was requested in read mode, or from where
...
.Note
====
The application may ask for data outside the bounds of the valid region, but
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the implementation context.
  * `[in]` _width_ - The image width in pixels.
    ...
    `<<VX_DF_IMAGE_YUYV,VX_DF_IMAGE_YUYV>>` must have even width.
  * `[in]` _height_ - The image height in pixels.
    The image in the formats of `<<VX_DF_IMAGE_NV12,VX_DF_IMAGE_NV12>>`,
    `<<VX_DF_IMAGE_NV21,VX_DF_IMAGE_NV21>>`, `<<VX_DF_IMAGE_IYUV,VX_DF_IMAGE_IYUV>>` must have even height.
  * `[in]` _color_ - The `<<VX_DF_IMAGE,VX_DF_IMAGE>>` (`<<vx_df_image_e>>`) code that
    represents the format of the image and the color space.
...
multi-planar images, as listed below.
...
*Parameters*
  ...
  * `[in]` _img_ - The reference to the parent image.
  * `[in]` _channel_ - The `<<vx_channel_e>>` channel to use.

*Returns:* An image reference `<<vx_image>>` to the sub-image.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the implementation context.
  * `[in]` _color_ - See the `<<vx_df_image_e>>` codes.
  ...
  * `[in]` _memory_type_ - `<<vx_memory_type_e>>`.
    ...
    HOST accessible pointers to memory.
...
image, even if the image was created via `<<vxCreateImageFromHandle>>`.
...
information.
_addr_ and _ptrs_ can then simply be application's local variables).
...
In other words, _stride_x_ should be sufficiently large so that there is no
overlap between data elements corresponding to different pixels, and
_stride_y_ {geq} _stride_x_ * _dim_x_.
====
...
*Parameters*
  ...
  * `[in]` _img_ - The reference to the parent image.
  * `[in]` _rect_ - The region of interest rectangle.
...
all pixels.
The uniform image created is read-only.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the parent graph.
  * `[in]` _width_ - The width of the image in pixels.
    A value of zero informs the interface that the value is unspecified.
    The image in the formats of `<<VX_DF_IMAGE_NV12,VX_DF_IMAGE_NV12>>`,
    `<<VX_DF_IMAGE_NV21,VX_DF_IMAGE_NV21>>`, `<<VX_DF_IMAGE_IYUV,VX_DF_IMAGE_IYUV>>`, `<<VX_DF_IMAGE_UYVY,VX_DF_IMAGE_UYVY>>`,
    `<<VX_DF_IMAGE_YUYV,VX_DF_IMAGE_YUYV>>` must have even width.
  * `[in]` _height_ - The height of the image in pixels.
    A value of zero informs the interface that the value is unspecified.
    The image in the formats of `<<VX_DF_IMAGE_NV12,VX_DF_IMAGE_NV12>>`,
    `<<VX_DF_IMAGE_NV21,VX_DF_IMAGE_NV21>>`, `<<VX_DF_IMAGE_IYUV,VX_DF_IMAGE_IYUV>>` must have even height.
  * `[in]` _color_ - The `<<VX_DF_IMAGE,VX_DF_IMAGE>>` (`<<vx_df_image_e>>`) code that
...
*Parameters*
  ...
  * `[in]` _index_ - The 0 based index of the pixel count in the patch.
    Indexes increase horizontally by 1 then wrap around to the next row.
  * `[in]` _addr_ - The pointer to the addressing mode information returned
    from `<<vxMapImagePatch>>`.
...
*Parameters*
  ...
  * `[in]` _ptr_ - The base pointer of the patch as returned from
    `<<vxMapImagePatch>>`.
  * `[in]` _x_ - The x dimension within the patch.
  * `[in]` _y_ - The y dimension within the patch.
  * `[in]` _addr_ - The pointer to the addressing mode information returned
    from `<<vxMapImagePatch>>`.
...
*Parameters*
  ...
  * `[in]` _image_ - The image from which to retrieve the valid region.
  * `[out]` _rect_ - The destination rectangle.
...
*Parameters*
  ...
  * `[in]` _image_ - The reference to the image object that contains the
    patch to map.
  * `[in]` _rect_ - The coordinates of image patch.
    The patch must be within the bounds of the image.
  ...
  ** (*map_id) must eventually be provided as the _map_id_ parameter of a
     call to `<<vxUnmapImagePatch>>`.
  * `[out]` _addr_ - The address of a structure describing the memory layout
    of the image patch to access.
    ...
    0, _stride_y_ > 0 and _stride_y_ {geq} _stride_x_ * _dim_x_.
  ...
  * `[in]` _usage_ - This declares the access mode for the image patch,
    ...
    For uniform images, only `<<VX_READ_ONLY,VX_READ_ONLY>>` is supported.
  ** `<<VX_READ_ONLY,VX_READ_ONLY>>`: after the function call, the content of the memory
     location pointed by (*_ptr_) contains the image patch data.
     Writing into this memory location is forbidden and its behavior is
     undefined.
  ** `<<VX_READ_AND_WRITE,VX_READ_AND_WRITE>>`: after the function call, the content of the
     memory location pointed by (*_ptr_) contains the image patch data;
     ...
     Writing into a gap between pixels (when _addr_\->__stride_x__ > pixel
     size in bytes or _addr_\->__stride_y__ > _addr_\->__stride_x__ *
     _addr_\->__dim_x__) is forbidden and its behavior is undefined.
  ** `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>`: after the function call, the memory location
     pointed by (*_ptr_) contains undefined data; writing each pixel of the
     patch is required prior to unmapping.
     Pixels not written by the application before unmap will become
     undefined after unmap, even if they were well defined before map.
     Like for `<<VX_READ_AND_WRITE,VX_READ_AND_WRITE>>`, writing into a gap between pixels is
     forbidden and its behavior is undefined.
  * `[in]` _mem_type_ - A `<<vx_memory_type_e>>` enumeration that specifies
    the type of the memory where the image patch is requested to be mapped.
  * `[in]` _flags_ - An integer that allows passing options to the map
    operation.
    Use the `<<vx_map_flag_e>>` enumeration.
...
.Note
====
The user may ask for data outside the bounds of the valid region, but such
data has an undefined value.
====
...
*Parameters*
  ...
  * `[in]` _image_ - The reference to the image to query.
  * `[in]` _attribute_ - The attribute to query.
...
// Brief text:
Releases a reference to an image object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _image_ - The pointer to the image to release.
...
*Parameters*
  ...
  * `[in]` _ptr_ - The pointer to the location from which to read the value.
  * `[in]` _size_ - The size in bytes of the object pointed to by _ptr_.
...
*Parameters*
  ...
  * `[in]` _image_ - The reference to the image to initialize.
  * `[in]` _pixel_value_ - The pointer to the constant pixel value to
    initialize all image pixels.
    See `<<vx_pixel_value_t>>`.
...
*Parameters*
  ...
  * `[in]` _image_ - The reference to the image.
  * `[in]` _rect_ - The value to be set to the image valid rectangle.
    A `NULL` indicates that the valid region is the entire image.
...
*Parameters*
  ...
  * `[in]` _image_ - The reference to an image created from handle
  * `[in]` _new_ptrs[]_ - pointer to a caller owned array that contains the
    new image handle (image plane pointers)
  ...
  ** _new_ptrs_ is `NULL`: the previous image storage memory is reclaimed by
     the caller, while no new handle is provided.
  * `[out]` _prev_ptrs[]_ - pointer to a caller owned array in which the
    application returns the previous image handle
  ** _prev_ptrs_ is non-`NULL`.
     ...
     address of the previous storage memory for plane i.
  ** _prev_ptrs_ is `NULL`: the previous handle is not returned.
  * `[in]` _num_planes_ - Number of planes in the image.
    ...
    are ignored.
    If either array is smaller than _num_planes_, the results are undefined.
...
*Parameters*
  ...
  * `[in]` _image_ - The reference to the image object to unmap.
...
*Enumerator*
  ...
  * [[VX_LUT_TYPE]] `VX_LUT_TYPE` - Indicates the value type of the LUT.
    Read-only.
    Use a `<<vx_enum>>`.
...
`<<VX_TYPE_UINT8,VX_TYPE_UINT8>>`, and (vx_uint32)(count/2) for `<<VX_TYPE_INT16,VX_TYPE_INT16>>`.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the context.
  * `[in]` _data_type_ - The type of data stored in the LUT.
  * `[in]` _count_ - The number of entries desired.
...
*Returns:* An LUT reference `<<vx_lut>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _lut_ - The reference to the LUT object to map.
  * `[out]` _map_id_ - The address of a `<<vx_map_id>>` variable where the
    function returns a map identifier.
  ** (*map_id) must eventually be provided as the map_id parameter of a call
     to `<<vxUnmapLUT>>`.
  * `[out]` _ptr_ - The address of a pointer that the function sets to the
    ...
    of elements equal to the value returned via `<<VX_LUT_COUNT,VX_LUT_COUNT>>`.
    Accessing the memory out of the bound of this array is forbidden and has
    an undefined behavior.
    The returned (*_ptr_) address is only valid between the call to the
    function and the corresponding call to `<<vxUnmapLUT>>`.
  * `[in]` _usage_ - This declares the access mode for the LUT, using the
    `<<vx_accessor_e>>` enumeration.
  ** `<<VX_READ_ONLY,VX_READ_ONLY>>`: after the function call, the content of the memory
     location pointed by (*_ptr_) contains the LUT data.
     Writing into this memory location is forbidden and its behavior is
     undefined.
  ** `<<VX_READ_AND_WRITE,VX_READ_AND_WRITE>>`: after the function call, the content of the
     ...
     LUT is unmapped.
  ** `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>`: after the function call, the memory location
     pointed by(*_ptr_) contains undefined data; writing each entry of LUT
     is required prior to unmapping.
     Entries not written by the application before unmap will become
     undefined after unmap, even if they were well defined before map.
  * `[in]` _mem_type_ - A `<<vx_memory_type_e>>` enumeration that specifies
    the type of the memory where the LUT is requested to be mapped.
  * `[in]` _flags_ - An integer that allows passing options to the map
    operation.
    Use 0 for this option.
...
// Brief text:
Releases a reference to a LUT object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _lut_ - The pointer to the LUT to release.
...
// Brief text:
Unmap and commit potential changes to LUT object that was previously mapped.
...
*Parameters*
  ...
  * `[in]` _lut_ - The reference to the LUT object to unmap.
  * `[out]` _map_id_ - The unique map identifier that was returned when
    calling `<<vxMapLUT>>`.
...
// Brief text:
The Matrix Object.
An MxN matrix of some unit type.
...
*Enumerator*
  ...
  * [[VX_MATRIX_TYPE]] `VX_MATRIX_TYPE` - The value type of the matrix.
    Read-only.
    Use a `<<vx_enum>>` parameter.
  * [[VX_MATRIX_ROWS]] `VX_MATRIX_ROWS` - The M dimension of the matrix.
    Read-only.
    Use a `<<vx_size>>` parameter.
  ...
  * [[VX_MATRIX_ORIGIN]] `VX_MATRIX_ORIGIN` - The origin of the matrix with
    a default value of
    [floor(`VX_MATRIX_COLUMNS`/2),floor(`VX_MATRIX_ROWS`/2)]
    Read-only.
...
*Parameters*
  ...
  * `[in]` _user_ptr_ - The address of the memory location where to store
    ...
    columns corresponding to `<<VX_MATRIX_COLUMNS,VX_MATRIX_COLUMNS>>`.
    The accessible memory must be large enough to contain this 2D array:
    accessible memory in bytes {geq} sizeof(data_element) * rows * columns.
  * `[in]` _usage_ - This declares the effect of the copy with regard to the
    matrix object using the `<<vx_accessor_e>>` enumeration.
    Only `<<VX_READ_ONLY,VX_READ_ONLY>>` and `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>` are supported:
...
*Parameters*
  ...
  * `[in]` _c_ - The reference to the overall context.
  * `[in]` _data_type_ - The unit format of the matrix.
    `<<VX_TYPE_UINT8,VX_TYPE_UINT8>>` or `<<VX_TYPE_INT32,VX_TYPE_INT32>>` or `<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>`.
  * `[in]` _columns_ - The first dimensionality.
  * `[in]` _rows_ - The second dimensionality.

*Returns:* An matrix reference `<<vx_matrix>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _pattern_ - The pattern of the matrix.
    See `<<VX_MATRIX_PATTERN,VX_MATRIX_PATTERN>>`.
  * `[in]` _columns_ - The first dimensionality.
  * `[in]` _rows_ - The second dimensionality.

*Returns:* A matrix reference `<<vx_matrix>>` of type `<<VX_TYPE_UINT8,VX_TYPE_UINT8>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
It supports the patterns as described below:
  ...
  * `<<VX_PATTERN_CROSS,VX_PATTERN_CROSS>>` is a matrix with dimensions equal to the given
    number of rows and columns, which both must be odd numbers.
    All cells in the center row and center column are equal to 255, and the
    rest are equal to zero.
    Dimensions of 3x3 and 5x5 must be supported.
...
The behavior when attempting to modify such a matrix is undefined.
...
*Parameters*
  ...
  * `[in]` _columns_ - The first dimensionality.
  * `[in]` _rows_ - The second dimensionality.
  * `[in]` _origin_col_ - The origin (first dimensionality).
  * `[in]` _origin_row_ - The origin (second dimensionality).
...
*Parameters*
  ...
  * `[in]` _mat_ - The matrix object to set.
  * `[in]` _attribute_ - The attribute to query.
...
*Parameters*
  ...
  * `[in]` _mat_ - The matrix reference to release.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _levels_ - The number of levels desired.
    This is required to be a non-zero value.
  * `[in]` _scale_ - Used to indicate the scale between pyramid levels.
    ...
    supported.
  * `[in]` _width_ - The width of the 0th level image in pixels.
  * `[in]` _height_ - The height of the 0th level image in pixels.
  * `[in]` _format_ - The format of all images in the pyramid.
    NV12, NV21, IYUV, UYVY and YUYV formats are not supported.

*Returns:* A pyramid reference `<<vx_pyramid>>` containing the sub-images.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Returns:* A pyramid reference `<<vx_pyramid>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _pyr_ - The pyramid to query.
  * `[in]` _attribute_ - The attribute for which to query.
...
// Brief text:
Releases a reference to a pyramid object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _pyr_ - The pointer to the pyramid to release.
...
A remap table contains per-pixel mapping of output pixels to input pixels.
...
*Enumerator*
  ...
  * [[VX_REMAP_SOURCE_WIDTH]] `VX_REMAP_SOURCE_WIDTH` - The source width.
    Read-only.
    Use a `<<vx_uint32>>` parameter.
  * [[VX_REMAP_SOURCE_HEIGHT]] `VX_REMAP_SOURCE_HEIGHT` - The source height.
    Read-only.
    Use a `<<vx_uint32>>` parameter.
...
for `<<VX_TYPE_COORDINATES2DF,VX_TYPE_COORDINATES2DF>>`).
...
after each line is determined by the _user_stride_y_ parameter.

*Parameters*

  * `[in]` _remap_ - The reference to the remap object that is the source or
    the destination of the patch copy.
  * `[in]` _rect_ - The coordinates of remap patch.
  ...
  * `[in]` _user_ptr_ - The address of the user memory location where to
    ...
    the copy was requested in write mode.
  ...
  ** `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>` means that data is copied into the remap object
     from the user memory.
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the type of the memory pointer by _user_ptr_.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _src_width_ - Width of the source image in pixel.
  * `[in]` _src_height_ - Height of the source image in pixels.
  * `[in]` _dst_width_ - Width of the destination image in pixels.
  * `[in]` _dst_height_ - Height of the destination image in pixels.

*Returns:* A remap reference `<<vx_remap>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[out]` _stride_y_ - The address of a vx_size variable where the
    ...
    of two successive lines in the mapped remap patch.
  ...
  * `[in]` _coordinate_type_ - This declares the type of the source
    coordinate data that the application wants to access in the remap patch.
    It must be `<<VX_TYPE_COORDINATES2DF,VX_TYPE_COORDINATES2DF>>`.
  * `[in]` _usage_ - This declares the access mode for the remap patch,
    using the `<<vx_accessor_e>>` enumeration.
  ** `<<VX_READ_ONLY,VX_READ_ONLY>>`: after the function call, the content of the memory
     location pointed by (*_ptr_) contains the remap patch data.
     Writing into this memory location is forbidden and its behavior is
     undefined.
  ** `<<VX_READ_AND_WRITE,VX_READ_AND_WRITE>>`: after the function call, the content of the
     memory location pointed by (*_ptr_) contains the remap patch data;
     ...
     is forbidden and its behavior is undefined.
  ** `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>`: after the function call, the memory location
     pointed by (*_ptr_) contains undefined data; writing each element of
     the patch is required prior to unmapping.
     Elements not written by the application before unmap will become
     undefined after unmap, even if they were well defined before map.
     Like for `<<VX_READ_AND_WRITE,VX_READ_AND_WRITE>>`, writing into a gap between element
     lines is forbidden and its behavior is undefined.
  * `[in]` _mem_type_ - A `<<vx_memory_type_e>>` enumeration that specifies
    the type of the memory where the remap patch is requested to be mapped.
...
// Brief text:
Releases a reference to a remap table object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _table_ - The pointer to the remap table to release.
...
*Parameters*
  ...
  * `[in]` _remap_ - The reference to the remap object to unmap.
  * `[out]` _map_id_ - The unique map identifier that was returned by
    `<<vxMapRemapPatch>>`.
...
*Enumerator*
  ...
  * [[VX_SCALAR_OP_AND]] `VX_SCALAR_OP_AND` - logical and.
  * [[VX_SCALAR_OP_OR]] `VX_SCALAR_OP_OR` - logical or.
  * [[VX_SCALAR_OP_XOR]] `VX_SCALAR_OP_XOR` - logical exclusive or.
  * [[VX_SCALAR_OP_NAND]] `VX_SCALAR_OP_NAND` - logical nand.
  * [[VX_SCALAR_OP_EQUAL]] `VX_SCALAR_OP_EQUAL` - comparison (equal).
  * [[VX_SCALAR_OP_NOTEQUAL]] `VX_SCALAR_OP_NOTEQUAL` - comparison (not
    equal).
  * [[VX_SCALAR_OP_LESS]] `VX_SCALAR_OP_LESS` - comparison (less than).
  ...
  * [[VX_SCALAR_OP_ADD]] `VX_SCALAR_OP_ADD` - arithmetic addition.
  * [[VX_SCALAR_OP_SUBTRACT]] `VX_SCALAR_OP_SUBTRACT` - arithmetic
    subtraction.
  * [[VX_SCALAR_OP_MULTIPLY]] `VX_SCALAR_OP_MULTIPLY` - arithmetic
    multiplication.
  * [[VX_SCALAR_OP_DIVIDE]] `VX_SCALAR_OP_DIVIDE` - arithmetic division.
  * [[VX_SCALAR_OP_MODULUS]] `VX_SCALAR_OP_MODULUS` - arithmetic (modulo
    operator).
  * [[VX_SCALAR_OP_MIN]] `VX_SCALAR_OP_MIN` - minimum of two scalars.
  * [[VX_SCALAR_OP_MAX]] `VX_SCALAR_OP_MAX` - maximum of two scalars.
...
*Parameters*
  ...
  * `[in]` _user_ptr_ - The address of the memory location where to store
    ...
    to `<<VX_SCALAR_TYPE,VX_SCALAR_TYPE>>`.
    The accessible memory must be large enough to contain this variable.
  * `[in]` _usage_ - This declares the effect of the copy with regard to the
    scalar object using the `<<vx_accessor_e>>` enumeration.
    Only `<<VX_READ_ONLY,VX_READ_ONLY>>` and `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>` are supported:
...
*Parameters*
  ...
  * `[in]` _scalar_ - The scalar object.
  * `[in]` _attribute_ - The enumeration to query.
    Use a `<<vx_scalar_attribute_e>>` enumeration.
  * `[out]` _ptr_ - The location at which to store the resulting value.
  * `[in]` _size_ - The size of the container to which _ptr_ points.
...
// Brief text:
Releases a reference to a scalar object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _scalar_ - The pointer to the scalar to release.
...
*Enumerator*
  ...
  * [[VX_THRESHOLD_INPUT_FORMAT]] `VX_THRESHOLD_INPUT_FORMAT` - The input
    image format the threshold was created for.
    Read-only.
    Use a `<<vx_enum>>` parameter.
...
*Parameters*
  ...
  * `[inout]` _true_value_ptr_ - The address of the memory location where to
    store the true output value if the copy was requested in read mode, or
  ...
  * `[inout]` _false_value_ptr_ - The address of the memory location where
    to store the false output value if the copy was requested in read mode,
    or from where to get the false output value to store into the threshold
    object if the copy was requested in write mode.
  * `[in]` _usage_ - This declares the effect of the copy with regard to the
    threshold object using the `<<vx_accessor_e>>` enumeration.
    Only `<<VX_READ_ONLY,VX_READ_ONLY>>` and `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>` are supported:
  ** `<<VX_READ_ONLY,VX_READ_ONLY>>` means that true and false output values are copied
     from the threshold object into the user memory.
...
object with type `<<VX_THRESHOLD_TYPE_RANGE,VX_THRESHOLD_TYPE_RANGE>>`.
...
*Parameters*
  ...
  * `[inout]` _lower_value_ptr_ - The address of the memory location where
    ...
    mode, or from where to get the lower thresholding value to store into
    the threshold object if the copy was requested in write mode.
  * `[inout]` _upper_value_ptr_ - The address of the memory location where
    ...
    mode, or from where to get the upper thresholding value to store into
    the threshold object if the copy was requested in write mode.
  * `[in]` _usage_ - This declares the effect of the copy with regard to the
    threshold object using the `<<vx_accessor_e>>` enumeration.
    Only `<<VX_READ_ONLY,VX_READ_ONLY>>` and `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>` are supported:
  ** `<<VX_READ_ONLY,VX_READ_ONLY>>` means that thresholding values are copied from the
     threshold object into the user memory.
...
object with type `<<VX_THRESHOLD_TYPE_BINARY,VX_THRESHOLD_TYPE_BINARY>>`.
...
*Parameters*
  ...
  * `[inout]` _value_ptr_ - The address of the memory location where to
    store the thresholding value if the copy was requested in read mode, or
  ...
  ** `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>` means the field of the (*_value_ptr_) union
     ...
     into the threshold object.
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the type of the memory referenced by _value_ptr_.
...
default 'true' value should be non-zero.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the context in which the object is
    created.
  * `[in]` _thresh_type_ - The type of thresholding operation.
...
*Returns:* A threshold reference `<<vx_threshold>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _thresh_ - The threshold object to set.
  * `[in]` _attribute_ - The attribute to query.
...
// Brief text:
Releases a reference to a threshold object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _thresh_ - The pointer to the threshold to release.
...
*Parameters*
  ...
  * `[in]` _attribute_ - The attribute to modify.
    Use a `<<vx_threshold_attribute_e>>` enumeration.
  * `[in]` _ptr_ - The pointer to the value to which to set the attribute.
  * `[in]` _size_ - The size of the data pointed to by _ptr_.
...
thus it is not allowed to be dimensionless or formatless.

*Parameters*

  * `[in]` _context_ - The reference to the overall Context.
  * `[in]` _exemplar_ - The exemplar object that defines the metadata of the
    created objects in the ObjectArray.
  * `[in]` _count_ - Number of Objects to create in the ObjectArray.
    This value must be greater than zero.

*Returns:* An ObjectArray reference `<<vx_object_array>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Returns:* A ObjectArray reference `<<vx_object_array>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
release the Object for each call to this function.

*Parameters*

  * `[in]` _arr_ - The ObjectArray.
  * `[in]` _index_ - The index of the object in the ObjectArray.
...
*Parameters*
  ...
  * `[in]` _arr_ - The reference to the ObjectArray.
  * `[in]` _attribute_ - The attribute to query.
...
*Parameters*
  ...
  * `[in]` _arr_ - The pointer to the ObjectArray to release.
...
dimensions, although a vendor can choose to support more dimensions in his
implementation.
The maximum number of dimensions supported by a given implementation can be
queried via the context attribute `<<VX_CONTEXT_MAX_TENSOR_DIMS,VX_CONTEXT_MAX_TENSOR_DIMS>>`.
Implementations must support tensors from one dimension (i.e., vectors)
through `<<VX_CONTEXT_MAX_TENSOR_DIMS,VX_CONTEXT_MAX_TENSOR_DIMS>>`, inclusive.
The individual elements of the tensor object may be any numerical data type.
...
*Enumerator*
  ...
  * [[VX_TENSOR_NUMBER_OF_DIMS]] `VX_TENSOR_NUMBER_OF_DIMS` - Number of
    dimensions.
  * [[VX_TENSOR_DIMS]] `VX_TENSOR_DIMS` - Dimension sizes.
...
*Parameters*
  ...
  * `[in]` _tensor_ - The reference to the tensor object that is the source
    or the destination of the copy.
  * `[in]` _number_of_dims_ - Number of patch dimension.
    ...
    assumed.
  * `[in]` _view_start_ - Array of patch start points in each dimension
  * `[in]` _view_end_ - Array of patch end points in each dimension
  * `[in]` _user_stride_ - Array of user memory strides in each dimension
  * `[in]` _user_ptr_ - The address of the memory location where to store
    the requested data if the copy was requested in read mode, or from where
...
The layout of the user memory must follow a row major order.
  * `[in]` _usage_ - This declares the effect of the copy with regard to the
    tensor object using the `<<vx_accessor_e>>` enumeration.
    Only `<<VX_READ_ONLY,VX_READ_ONLY>>` and `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>` are supported:
...
adjacent 2D images or not depending on the stride value.
...
*Parameters*
  ...
  * `[in]` _tensor_ - The tensor data from which to extract the images.
    Has to be a 3d tensor.
  * `[in]` _rect_ - Image coordinates within tensor data.
  * `[in]` _array_size_ - Number of images to extract.
  * `[in]` _jump_ - Delta between two images in the array.
  * `[in]` _image_format_ - The requested image format.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the implementation context.
  * `[in]` _number_of_dims_ - The number of dimensions.
  * `[in]` _dims_ - Dimensions sizes in elements.
  * `[in]` _data_type_ - The `<<vx_type_e>>` that represents the data type
    of the tensor data elements.
  * `[in]` _fixed_point_position_ - Specifies the fixed point position when
    the input element type is integer.
    if 0, calculations are performed in integer math.

*Returns:* A tensor data reference.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
// Brief text:
Creates a tensor data from another tensor data given a view.
...
*Parameters*
  ...
  * `[in]` _tensor_ - The reference to the parent tensor data.
  * `[in]` _number_of_dims_ - Number of dimensions in the view.
    Error return if 0 or greater than number of tensor dimensions.
    If smaller than number of tensor dimensions, the lower dimensions are
    assumed.
  * `[in]` _view_start_ - View start coordinates
  * `[in]` _view_end_ - View end coordinates

*Returns:* The reference to the sub-tensor.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Returns:* A tensor data reference.Any possible errors preventing a
...
*Parameters*
  ...
  * `[in]` _tensor_ - The reference to the tensor data to query.
  * `[in]` _attribute_ - The attribute to query.
...
// Brief text:
Releases a reference to a tensor data object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _tensor_ - The pointer to the tensor data to release.
...
*Return Values*
  ...
  * `<<VX_SUCCESS,VX_SUCCESS>>` - No errors; all other values indicate failure
  * * - An error occurred.
    See `<<vx_status_e>>`.
...
*Parameters*
  * `[in]` _context_ - The reference to the implementation context.
  * `[in]` _size_ - The size of user struct in bytes.

*Returns:* A `<<vx_enum>>` value that is a type given to the User to refer
to their custom structure when declaring a `<<vx_array>>` of that structure.

*Return Values*

  * `<<VX_TYPE_INVALID,VX_TYPE_INVALID>>` - If the namespace of types has been exhausted.
...
*Parameters*
  ...
  * `[in]` _graph_ - The reference to the graph in which this node exists.
  * `[in]` _kernel_ - The kernel reference to associate with this new node.
...
// Detailed text:
If the indicated border mode is not supported, an error
...
*Enumerator*
  ...
  * [[VX_DELAY_TYPE]] `VX_DELAY_TYPE` - The type of objects in the delay.
    Read-only.
    Use a `<<vx_enum>>` parameter.
  * [[VX_DELAY_SLOTS]] `VX_DELAY_SLOTS` - The number of items in the delay.
    Read-only.
    Use a `<<vx_size>>` parameter.
...
forth until index [eq]#-count + 1#.
The data originally at index [eq]#-count + 1# move to index 0.
Here [eq]#count# is the number of slots in delay ring.
...
// Detailed text:
This function creates a delay object with _num_slots_ slots.
Each slot contains a clone of the exemplar.
The clones only inherit the metadata of the exemplar.
The data content of the exemplar is ignored and the clones have their data
undefined at delay creation time.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the context.
  * `[in]` _exemplar_ - The exemplar object.
    Supported exemplar object types are:
  ...
  * `<<VX_TYPE_THRESHOLD,VX_TYPE_THRESHOLD>>`
  * `<<VX_TYPE_TENSOR,VX_TYPE_TENSOR>>`
  * `[in]` _num_slots_ - The number of objects in the delay.
    This value must be greater than zero.

*Returns:* A delay reference `<<vx_delay>>`.
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
.Note
====
The delay index is in the range [eq]#[-count + 1, 0]#.
0 is always the _current_ object.
====
...
*Parameters*
  ...
  * `[in]` _delay_ - The reference to a delay object.
  * `[in]` _attribute_ - The attribute to query.
...
// Brief text:
Releases a reference to a delay object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _delay_ - The pointer to the delay object reference to release.
...
*Enumerator*
  ...
  * [[VX_KERNEL_PARAMETERS]] `VX_KERNEL_PARAMETERS` - Queries a kernel for
    the number of parameters the kernel supports.
    Read-only.
    Use a `<<vx_uint32>>` parameter.
  ...
  * [[VX_KERNEL_LOCAL_DATA_SIZE]] `VX_KERNEL_LOCAL_DATA_SIZE` - The local
    data area allocated with each kernel when it becomes a node.
    Read-write.
...
When programming the parameters, use
  ...
  * `<<VX_BIDIRECTIONAL,VX_BIDIRECTIONAL>>` for [in,out]
...
--
  * [[VX_KERNEL_SOBEL_3x3]] `VX_KERNEL_SOBEL_3x3` - The
    Sobel 3x3 Filter Kernel.
+
--
// Detailed text
*See also:* <<Sobel 3x3>>
--
  * [[VX_KERNEL_MAGNITUDE]] `VX_KERNEL_MAGNITUDE` - The Magnitude Kernel.
+
--
// Detailed text
...
*See also:* <<Magnitude>>
--
  * [[VX_KERNEL_PHASE]] `VX_KERNEL_PHASE` - The Phase Kernel.
+
--
// Detailed text
...
--
  * [[VX_KERNEL_HISTOGRAM]] `VX_KERNEL_HISTOGRAM` - The Histogram Kernel.
+
--
// Detailed text
*See also:* <<Histogram>>
...
--
  * [[VX_KERNEL_THRESHOLD]] `VX_KERNEL_THRESHOLD` - The Threshold Kernel.
+
--
// Detailed text
...
--
  * [[VX_KERNEL_GAUSSIAN_PYRAMID]] `VX_KERNEL_GAUSSIAN_PYRAMID` - The
    gaussian image pyramid kernel.
+
--
// Detailed text
*See also:* <<Gaussian Image Pyramid>>
--
  * [[VX_KERNEL_ACCUMULATE]] `VX_KERNEL_ACCUMULATE` - The accumulation
    kernel.
+
--
// Detailed text
*See also:* <<Accumulate>>
--
  * [[VX_KERNEL_ACCUMULATE_WEIGHTED]] `VX_KERNEL_ACCUMULATE_WEIGHTED` - The
    weigthed accumulation kernel.
+
--
// Detailed text
*See also:* <<Accumulate Weighted>>
--
  * [[VX_KERNEL_ACCUMULATE_SQUARE]] `VX_KERNEL_ACCUMULATE_SQUARE` - The
    squared accumulation kernel.
+
--
// Detailed text
...
--
  * [[VX_KERNEL_AND]] `VX_KERNEL_AND` - The Bitwise And Kernel.
+
--
// Detailed text
*See also:* <<Bitwise AND>>
--
  * [[VX_KERNEL_OR]] `VX_KERNEL_OR` - The Bitwise Inclusive Or Kernel.
+
--
// Detailed text
*See also:* <<Bitwise INCLUSIVE OR>>
--
  * [[VX_KERNEL_XOR]] `VX_KERNEL_XOR` - The Bitwise Exclusive Or Kernel.
+
--
// Detailed text
*See also:* <<Bitwise EXCLUSIVE OR>>
--
  * [[VX_KERNEL_NOT]] `VX_KERNEL_NOT` - The Bitwise Not Kernel.
+
--
// Detailed text
...
--
  * [[VX_KERNEL_ADD]] `VX_KERNEL_ADD` - The Addition Kernel.
+
--
// Detailed text
*See also:* <<Arithmetic Addition>>
--
  * [[VX_KERNEL_SUBTRACT]] `VX_KERNEL_SUBTRACT` - The Subtraction Kernel.
+
--
// Detailed text
...
--
  * [[VX_KERNEL_OPTICAL_FLOW_PYR_LK]] `VX_KERNEL_OPTICAL_FLOW_PYR_LK` - The
    Optical Flow Pyramid (LK) Kernel.
+
--
// Detailed text
*See also:* <<Optical Flow Pyramid (LK)>>
--
  * [[VX_KERNEL_REMAP]] `VX_KERNEL_REMAP` - The Remap Kernel.
+
--
// Detailed text
*See also:* <<Remap>>
...
--
  ...
  * [[VX_KERNEL_LAPLACIAN_PYRAMID]] `VX_KERNEL_LAPLACIAN_PYRAMID` - The
    Laplacian Image Pyramid Kernel.
+
--
// Detailed text
...
--
  * [[VX_KERNEL_LBP]] `VX_KERNEL_LBP` - The LBP Kernel.
+
--
// Detailed text
*See also:* <<LBP>>
...
--
  * [[VX_KERNEL_TENSOR_ADD]] `VX_KERNEL_TENSOR_ADD` - The tensor add Kernel.
+
--
// Detailed text
*See also:* <<Tensor Add>>
...
--
  * [[VX_KERNEL_COPY]] `VX_KERNEL_COPY` - The data object copy kernel.
+
--
// Detailed text
...
*See also:* <<HOG>>
--
  * [[VX_KERNEL_HOG_CELLS]] `VX_KERNEL_HOG_CELLS` - The HOG Cells kernel.
+
--
// Detailed text
*See also:* <<HOG>>
...
--
  * [[VX_KERNEL_SELECT]] `VX_KERNEL_SELECT` - The select kernel.
+
--
// Detailed text
*See also:* <<Control Flow>>
--
  ...
  * [[VX_KERNEL_MAX]] `VX_KERNEL_MAX` - The max kernel.
+
--
// Detailed text
*See also:* <<Max>>
--
  * [[VX_KERNEL_MIN]] `VX_KERNEL_MIN` - The min kernel.
+
--
// Detailed text
*See also:* <<Min>>
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the implementation context.
  * `[in]` _name_ - The string of the name of the kernel to get.

*Returns:* A kernel reference.
Any possible errors preventing a successful completion of the function
should be checked using `<<vxGetStatus>>`.
...
.Note
====
User Kernels should follow a "`dotted`" hierarchical syntax.
For example: "com.company.example.xyz".
====
...
number of parameters, enum values, etc.
...
*Parameters*
  ...
  * `[in]` _kernel_ - The kernel reference to query.
  * `[in]` _attribute_ - The attribute to query.
...
*Parameters*
  ...
  * `[in]` _kernel_ - The pointer to the kernel reference to release.
...
// @details
An abstract input, output, or bidirectional data object passed to a computer
vision function.
...
*Enumerator*
  ...
  * [[VX_BIDIRECTIONAL]] `VX_BIDIRECTIONAL` - The parameter is both an input
    and output.
...
*Enumerator*
  ...
  * [[VX_PARAMETER_INDEX]] `VX_PARAMETER_INDEX` - Queries a parameter for
    its index value on the kernel with which it is associated.
    Read-only.
    Use a `<<vx_uint32>>` parameter.
...
*Parameters*
  ...
  * `[in]` _kernel_ - The reference to the kernel.
  * `[in]` _index_ - The index of the parameter.

*Returns:* A `<<vx_parameter>>` reference.
Any possible errors preventing a successful completion of the function
should be checked using `<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _node_ - The node from which to extract the parameter.
  * `[in]` _index_ - The index of the parameter to which to get a reference.

*Returns:* A parameter reference `<<vx_parameter>>`.
Any possible errors preventing a successful completion of the function
should be checked using `<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _parameter_ - The reference to the parameter.
  * `[in]` _attribute_ - The attribute to query.
...
// Brief text:
Releases a reference to a parameter object.
The object may not be garbage collected until its total reference count is
zero.
...
*Parameters*
  ...
  * `[in]` _param_ - The pointer to the parameter to release.
...
*Parameters*
  ...
  * `[in]` _node_ - The node that contains the kernel.
  * `[in]` _index_ - The index of the parameter desired.
  * `[in]` _value_ - The desired value of the parameter.
...
*Parameters*
  ...
  * `[in]` _parameter_ - The reference to the kernel parameter.
  * `[in]` _value_ - The value to associate with the kernel parameter.
...
processing should proceed.
  ...
  * If `<<VX_ACTION_ABANDON,VX_ACTION_ABANDON>>` is returned, execution is unspecified for all
    nodes for which this node is a dominator.
    Nodes that are dominators of this node will have executed.
...
*Parameters*
  ...
  * `[in]` _node_ - The node to which the callback was attached.
...
*Enumerator*
  ...
  * [[VX_ACTION_CONTINUE]] `VX_ACTION_CONTINUE` - Continue executing the
    graph with no changes.
  * [[VX_ACTION_ABANDON]] `VX_ACTION_ABANDON` - Stop executing the graph.
...
*Parameters*
  ...
  * `[in]` _node_ - The reference to the node.
...
*Return Values*
  ...
  * `<<VX_SUCCESS,VX_SUCCESS>>` - Callback assigned; any other value indicates failure.
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - node is not a valid `<<vx_node>>`
    reference.
...
*Parameters*
  ...
  * `[in]` _ref_ - The reference to add the log entry against.
    Some valid value must be provided.
  * `[in]` _status_ - The status code.
    `<<VX_SUCCESS,VX_SUCCESS>>` status entries are ignored and not added.
  * `[in]` _message_ - The human readable message to add to the log.
  * `[in]` _..._ - a list of variable arguments to the message.
...
*Parameters*
  ...
  * `[in]` _context_ - The overall context to OpenVX.
  * `[in]` _callback_ - The callback function.
    If `NULL`, the previous callback is removed.
...
*Parameters*
  ...
  * `[in]` _reference_ - The reference to the object to hint at.
    This could be `<<vx_context>>`, `<<vx_graph>>`, `<<vx_node>>`,
    `<<vx_image>>`, `<<vx_array>>`, or any other reference.
  * `[in]` _hint_ - A `<<vx_hint_e>>` _hint_ to give to a `<<vx_context>>`.
    This is a platform-specific optimization or implementation mechanism.
  * `[in]` _data_ - Optional vendor specific data.
  * `[in]` _data_size_ - Size of the data structure _data_.
...
*Parameters*
  ...
  * `[in]` _reference_ - The reference to the object to set the directive
    ...
    This could be `<<vx_context>>`, `<<vx_graph>>`, `<<vx_node>>`,
    `<<vx_image>>`, `<<vx_array>>`, or any other reference.
  * `[in]` _directive_ - The directive to set.
    See `<<vx_directive_e>>`.
...
// Brief text:
The pointer to the kernel deinitializer.
...
*Parameters*
  ...
  * `[in]` _parameters_ - The array of parameter references.
  * `[in]` _num_ - The number of parameters.
...
supplied to the callback to calculate the output valid rectangle.
...
*Parameters*
  ...
  * `[inout]` _node_ - The handle to the node that is being validated.
  ...
  * `[in]` _input_valid_ - A pointer to an array of valid regions of input
    images or images contained in image container (e.g. pyramids).
  ...
  * `[out]` _output_valid_ - An array of valid regions that should be set
    ...
    For a simple output image the array size is always one.
    Each rectangle supplies the valid region for one image.
    The array memory allocation is managed by the framework.
...
// Brief text:
The pointer to the kernel initializer.
...
// Brief text:
The user-defined kernel node parameters validation function.
...
*Parameters*
  ...
  * `[in]` _node_ - The handle to the node that is being validated.
  * `[in]` _parameters_ - The array of parameters to be validated.
  * `[in]` _num_ - Number of parameters to be validated.
  * `[in]` _metas_ - A pointer to a pre-allocated array of structure
    references that the system holds.
...
`<<vxLoadKernels>>` and unloaded by `<<vxUnloadKernels>>`.
...
*Enumerator*
  ...
  * [[VX_VALID_RECT_CALLBACK]] `VX_VALID_RECT_CALLBACK` - Valid rectangle
    callback during output parameter validation.
    Write-only.
...
*Parameters*
  ...
  * `[in]` _kernel_ - The reference to the kernel added with
    `<<vxAddUserKernel>>`.
  * `[in]` _index_ - The index of the parameter to add.
  * `[in]` _dir_ - The direction of the parameter.
    This must be either `<<VX_INPUT,VX_INPUT>>` or `<<VX_OUTPUT,VX_OUTPUT>>`.
    `<<VX_BIDIRECTIONAL,VX_BIDIRECTIONAL>>` is not supported for this function.
  * `[in]` _data_type_ - The type of parameter.
    This must be a value from `<<vx_type_e>>`.
  * `[in]` _state_ - The state of the parameter (required or not).
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the context the kernel must be added
    to.
  * `[in]` _name_ - The string to use to match the kernel.
  * `[in]` _enumeration_ - The enumerated value of the kernel to be used by
    clients.
  * `[in]` _func_ptr_ - The process-local function pointer to be invoked.
  * `[in]` _numParams_ - The number of parameters for this kernel.
  * `[in]` _validate_ - The pointer to `<<vx_kernel_validate_f>>`, which
    validates parameters to this kernel.
  * `[in]` _init_ - The kernel initialization function.
  * `[in]` _deinit_ - The kernel de-initialization function.
...
// Brief text:
Allocates and registers user-defined kernel enumeration to a context.
The allocated enumeration is from available pool of 4096 enumerations
reserved for dynamic allocation from `<<VX_KERNEL_BASE(VX_ID_USER,0)>>`.
...
specify individual kernel enum IDs in a header file, shown below:
...
the kernel is _ready_ to be used.
...
functions named `<<vxPublishKernels,vxPublishKernels>>` and `<<vxUnpublishKernels,vxUnpublishKernels>>`.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the context the kernels must be
    added to.
  * `[in]` _module_ - The short name of the module to load.
...
*Parameters*
  ...
  * `[in]` _kernel_ - The reference to the kernel.
  * `[in]` _attribute_ - The enumeration of the attributes.
...
*Parameters*
  ...
  * `[in]` _meta_ - The reference to the `<<vx_meta_format>>` struct to set
  ...
  * `[in]` _ptr_ - The input pointer of the value to set on the meta format
    object.
  * `[in]` _size_ - The size in bytes of the object to which _ptr_ points.
...
*Parameters*
  ...
  * `[in]` _meta_ - The meta format object to set
  * `[in]` _exemplar_ - The exemplar data object.
...
module using the `<<vxLoadKernels>>` function.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the context the kernels must be
    removed from.
  * `[in]` _module_ - The short name of the module to unload.
    On systems where there are specific naming conventions for modules, the
    name passed should ignore such conventions.
    For example: `libxyz.so` should be passed as just `xyz` and the
    implementation will _do the right thing_ that the platform requires.
...
parameters.
Clients can then create Graph creation methods (a.k.a.
_Graph Factories_).
When creating these factories, the client will typically not be able to use
...
*Parameters*
  ...
  * `[in]` _graph_ - The graph reference that contains the node.
...
// Brief text:
Sets a reference to the parameter on the graph.
...
*Parameters*
  ...
  * `[in]` _graph_ - The graph reference.
  * `[in]` _index_ - The parameter index.
  * `[in]` _value_ - The reference to set to the parameter.
