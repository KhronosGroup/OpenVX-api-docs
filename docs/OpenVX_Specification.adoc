// Copyright (c) 2016-2026 Khronos Group. This work is licensed under a
...
OpenVX is a low-level programming framework designed to enable software
...
is consistent with this specification are defined separately.
...
NOTE: `VX/vx_compatibility.h` is *not* included by `VX/vx.h`.
Applications that require access to deprecated identifiers must explicitly
add the following include *after* `VX/vx.h`:
...
that form coherent and useful subsets of the OpenVX API. Refer to the
https://www.khronos.org/registry/OpenVX[Khronos OpenVX Registry]
for this document.
...
extensions.
  ...
  * Defined Enumeration values are named as `VX___ENUMVALUE_VENDOR__`, e.g.,
    `VX_PARAM_STRUCT_ATTRIBUTE_SIZE_ACME` using only capital letters starting
...
Data objects are objects that are processed by graphs in nodes.
  ...
  * <<group_convolution>> An opaque object that contains an latexmath:[M \times N] matrix of `<<vx_int16>>` values.
  ...
  * <<group_matrix>> An opaque object that contains an latexmath:[M \times N]
...
creator functions when serious platform issues occur (i.e., out of memory or
out of handles).
...
different in the following respects.
  ...
  * Inaccessible - No calls to the Map/Unmap or Copy APIs shall succeed given
  ...
  * Scoped - Virtual data objects are scoped within the Graph in which they
    ...
    In other words, data content of a virtual object is implementation-defined before
...
For virtual images created with undefined or partially defined dimensions and/or without
defined format (`<<VX_DF_IMAGE_VIRT,VX_DF_IMAGE_VIRT>>`), the image dimensions and format
will be inherited from node input images during graph verification.
...
This means that given a constructed graph reference latexmath:[G], it may be
...
execution of Graphs.
The behavior of a latexmath:[\texttt{processGraph}(G)] call is determined by the
structure of the Processing Graph latexmath:[G].
The Processing Graph is a bipartite graph consisting of a set of Nodes
latexmath:[N_{1} \ldots N_{n}] and a set of data objects latexmath:[d_{1} \ldots d_{i}].
Each edge latexmath:[(N_{x},D_{y})] in the graph represents a data object latexmath:[D_{y}]
that is written by Node latexmath:[N_{x}] and each edge latexmath:[(D_{x},N_{y})]
represents a data object latexmath:[D_{x}] that is read by Node latexmath:[N_{y}].
Each edge latexmath:[e] has a name latexmath:[\texttt{Name}(e)], which gives the parameter name
...
In addition, the Processing Graph has several restrictions:
  ...
  . _Output typing_ - Every output edge latexmath:[(N_{x},D_{y})] requires
    latexmath:[\texttt{Type}(N_{x}, \texttt{Name}(N_{x},D_{y}))] in `{OUTPUT}`
  . _Input typing_ - Every input edge latexmath:[(D_{x},N_{y})] requires
    latexmath:[\texttt{Type}(N_{y}, \texttt{Name}(D_{x},N_{y}))] in `{INPUT}`
  . _Single Writer_ - Every data object is the target of at most one output
    edge.
  . _Broken Cycles_ - Every cycle in latexmath:[G] must contain at least input
    edge latexmath:[(D_{x},N_{y})] where latexmath:[D_{x}] is Delay.
  . _Virtual images must have a source_ - If latexmath:[D_{y}] is Virtual, then
    there is at least one output edge that writes latexmath:[D_{y}(N_{x},D_{y})]
  . _Delay data objects shall not be virtual_ - If latexmath:[D_{x}] is Delay then
...
Before the graph executes, the following initial marking is used:
  ...
  * All input edges latexmath:[(D_{x},N_{y})] from non-Virtual objects latexmath:[D_{x}]
    are marked (parameters must be set).
  * All input edges latexmath:[(D_{x},N_{y})] with an output edge latexmath:[(N_{z},D_{x})]
    are unmarked.
  * All input edges latexmath:[(D_{x},N_{y})] where latexmath:[D_{x}] is a Delay data
    object are marked.

Processing a node results in unmarking all the corresponding input edges and
marking all its output edges; marking an output edge latexmath:[(N_{x},D_{y})] where
latexmath:[D_{y}] is not a Delay results in marking all of the input edges
latexmath:[(D_{y},N_{z})].
Following these rules, it is possible to statically schedule the nodes in a
graph as follows: Construct a precedence graph latexmath:[P], including all the
nodes latexmath:[N_{1} \ldots N_{x}], and an edge latexmath:[(N_{x},N_{z})] for every pair
of edges latexmath:[(N_{x},D_{y})] and latexmath:[(D_{y},N_{z})] where latexmath:[D_{y}] is not a
Delay.
...
The following assertions should be verified:
  ...
  * latexmath:[P] is a Directed Acyclic Graph (DAG), implied by 4 and the way it
    is constructed.
...
parameter of node latexmath:[N_{1}] and input parameter of node latexmath:[N_{2}] in a
...
The following figure shows examples of this dependency.
To simplify subsequent definitions and requirements a limitation is imposed
that if a sub-image latexmath:[I^{'}] has been created from image latexmath:[I] and
sub-image latexmath:[I^{''}] has been created from latexmath:[I^{'}], then latexmath:[I^{''}] is
still considered a sub-image of latexmath:[I] and not of latexmath:[I^{'}].
...
the other one.
Therefore it implies that latexmath:[N_{1}] comes before latexmath:[N_{2}] in the graph's
topological order.
...
image::images/Overlap_DataObj_Image.svg[align="center",{halfimagewidth}]
  ...
  ** latexmath:[C_{0}(d) = \{d\}]
  ** latexmath:[C_{1}(d)] is the set of all data objects that are _directly
     contained_ by _d_:
  ...
  *** If _d_ is a delay object, all slots of _d_ are directly contained by
      _d_.
  ** For latexmath:[i > 1], latexmath:[C_{i}(d)] is the set of all data objects that are
...
++++
  ...
  . _Overlapping Relationship_ - The overlapping relation latexmath:[R_{ov}] is a
    relation defined for images, such that if latexmath:[i_{1}] and latexmath:[i_{2}] in
    _C(i)_, _i_ being an image, then latexmath:[i_{1} R_{ov} i_{2}] is true if and
    only if latexmath:[i_{1}] and latexmath:[i_{2}] overlap, i.e there exists a point
    (x,y) of _i_ that is contained in both latexmath:[i_{1}] and latexmath:[i_{2}].
    Note that this relation is reflexive and symmetric, but not transitive:
    latexmath:[i_{1}] overlaps latexmath:[i_{2}] and latexmath:[i_{2}] overlaps latexmath:[i_{3}] does
    not necessarily imply that latexmath:[i_{1}] overlaps latexmath:[i_{3}], as
...
--
  . _Dependency Relationship_ - The dependency relationship latexmath:[N_{1} \rightarrow N_{2}], is a relation defined for nodes.
    latexmath:[N_{1} \rightarrow N_{2}] means that latexmath:[N_{2}] depends on latexmath:[N_{1}]
    and then implies that latexmath:[N_{2}] must be executed after the completion
    of latexmath:[N_{1}].
  . latexmath:[N_{1} \rightarrow N_{2}] if latexmath:[N_{1}] writes to a data object
    latexmath:[d_{1}] and latexmath:[N_{2}] reads from a data object latexmath:[d_{2}] and:
+
--
...
[latexmath]
++++
d_{1} \in C(d_{2}) \text{ or } d_{2} \in C(d_{1}) \text{ or } (I(d_{1}) \text{ and } I(d_{2}) \text{ and } d_{1} R_{ov} d_{2})
++++
--

If data object latexmath:[D_{y}] of an output edge latexmath:[(N_{x},D_{y})] overlaps with
a data object latexmath:[D_{z}] then the result is implementation-defined.
...
to get a unique identifier (up to a limit of latexmath:[2^{12} - 1] vendors).
...
targets to assign nodes to (or to execute an immediate mode function).
...
Some objects support APIs to set attributes even though all their attributes are
specified as read-only in this specification. These APIs are meant for extending attributes.
...
*Parameters*
  ...
  * `[out]` _out_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format `[*REQ-0008*]`[[REQ-0008]].
...
*Parameters*
  ...
  * `[out]` _out_ - The output image, a `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` image `[*REQ-0020*]`[[REQ-0020]].
    The output image must have the same dimensions as the input image.
...
*Parameters*
  ...
  * `[out]` _out_ - The output image, a `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` image `[*REQ-0032*]`[[REQ-0032]].
    The output image must have the same dimensions as the input image.
...
The spatial dimensions use isotropic distance, which is Cartesian.
...
where latexmath:[x, y] are in the spatial euclidean space.
latexmath:[t, \tau] are vectors in radiometric space.
...
*Parameters*
  ...
  * `[in]` _diameter_ - Diameter of each pixel neighborhood that is used during
    filtering `[*REQ-0039*]`[[REQ-0039]].
    Values of _diameter_ must be odd.
    Greater than 3 and less than 10 `[*REQ-0040*]`[[REQ-0040]].
  * `[in]` _sigmaSpace_ - Filter sigma in the spatial space.
    Supported values are greater than 0 and less than or equal to 20 `[*REQ-0041*]`[[REQ-0041]].
  * `[in]` _sigmaValues_ - Filter sigma in the radiometric space.
    Supported values are greater than 0 and less than or equal to 20 `[*REQ-0042*]`[[REQ-0042]].
...
*Parameters*
  ...
  * `[in]` _sigmaSpace_ - Filter sigma in the spatial space.
    Supported values are greater than 0 and less than or equal to 20.
  * `[in]` _sigmaValues_ - Filter sigma in the radiometric space.
    Supported values are greater than 0 and less than or equal to 20.
  * `[out]` _dst_ - The output data, a `<<vx_tensor>>` of type
    `<<VX_TYPE_UINT8,VX_TYPE_UINT8>>` or `<<VX_TYPE_INT16,VX_TYPE_INT16>>`.
...
[latexmath]
++++
out(x,y) = in_{1}(x,y) \land in_{2}(x,y)
++++
...
*Parameters*
  ...
  * `[out]` _out_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>` output image `[*REQ-0049*]`[[REQ-0049]].
...
[latexmath]
++++
out(x,y) = in_{1}(x,y) \oplus in_{2}(x,y)
++++
...
*Parameters*
  ...
  * `[out]` _out_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>` output image `[*REQ-0055*]`[[REQ-0055]].
    The output image must have the same dimensions and same format as the input image.
...
[latexmath]
++++
out(x,y) = in_{1}(x,y) \lor in_{2}(x,y)
++++
...
*Parameters*
  ...
  * `[out]` _out_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>` output image `[*REQ-0061*]`[[REQ-0061]].
    The output image must have the same dimensions and same format as the input image.
...
*Parameters*
  ...
  * `[out]` _output_ - The `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>` output image `[*REQ-0066*]`[[REQ-0066]].
    The output image must have the same dimensions and same format as the input image.
...
*Parameters*
  ...
  * `[out]` _output_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` format `[*REQ-0071*]`[[REQ-0071]].
    The output image must have the same dimensions as the input image.
...
The two resulting directional gradient images (latexmath:[dx] and latexmath:[dy]) are
...
_norm_type_ parameter, so the magnitude may be latexmath:[|dx| + |dy|] for `<<VX_NORM_L1,VX_NORM_L1>>` or latexmath:[\sqrt{dx^{2} + dy^{2}}]
...
[latexmath]
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\mathbf{sobel}_y = transpose(\mathbf{sobel}_{x}) =
\begin{bmatrix}
  -1 & -2 & -1 \\
   0 &  0 &  0 \\
   1 &  2 &  1
...
[latexmath]
++++
\mathbf{sobel}_{y} = transpose(\mathbf{sobel}_{x})
++++
--
...
*Parameters*
  ...
  * `[in]` _plane2_ - [optional] The plane that forms channel 2.
    ...
    Use NULL if the output format does not require this channel.
...
*Parameters*
  ...
  * `[out]` _output_ - The output image.
    Must be `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` `[*REQ-0091*]`[[REQ-0091]].
    The output image must have the same dimensions as the input image.
...
table below `[*REQ-0093*]`[[REQ-0093]], where the columns are output types and the rows are input
types.
...
NOTE: `<<VX_DF_IMAGE_RGBA,VX_DF_IMAGE_RGBA>>` was added in version 1.3.2.
Color conversions involving RGBA are not defined in this version of the specification.
...
[latexmath]
++++
value_{integer} = max(0, min(255, floor(value_{real} \times 256.0)))
++++
...
[latexmath]
++++
value_{integer} = max(0, min(255, floor(value_{real} \times 256.0 + 128)))
++++
...
[latexmath]
++++
value_{integer} = max(0, min(255, floor(value_{real} \times 219.0 + 16.5)))
++++
...
[latexmath]
++++
value_{integer} = max(0, min(255, floor(value_{real} \times 224.0 + 128.5)))
++++
...
In all cases, for the purposes of conversion, these color representations
are interpreted as nonlinear in intensity, as defined by the BT.601, BT.709,
and sRGB specifications.
That is, the encoded color channels are nonlinear R^'^, G^'^ and B^'^,
Y^'^, P~b~, and P~r~.
...
[latexmath]
++++
G_{BT601_525} = R_{BT709} \times -0.019633 + G_{BT709} \times 1.036363 + B_{BT709} \times -0.016731
++++
[latexmath]
++++
B_{BT601_525} = R_{BT709} \times 0.001632 + G_{BT709} \times 0.004412 + B_{BT709} \times 0.993956
++++

[latexmath]
++++
R_{BT601_625} = R_{BT601_525} \times 0.900657 + G_{BT601_525} \times 0.088807 + B_{BT601_525} \times 0.010536
++++
[latexmath]
++++
G_{BT601_625} = R_{BT601_525} \times 0.017772 + G_{BT601_525} \times 0.965793 + B_{BT601_525} \times 0.016435
++++
[latexmath]
++++
B_{BT601_625} = R_{BT601_525} \times -0.001853 + G_{BT601_525} \times -0.015948 + B_{BT601_525} \times 1.017801
++++

[latexmath]
++++
R_{BT601_625} = R_{BT709} \times 0.957815 + G_{BT709} \times 0.042185
++++
[latexmath]
++++
...
[latexmath]
++++
B_{BT601_625} = G_{BT709} \times -0.011934 + B_{BT709} \times 1.011934
++++

[latexmath]
++++
R_{BT709} = R_{BT601_525} \times 0.939542 + G_{BT601_525} \times 0.050181 + B_{BT601_525} \times 0.010277
++++
[latexmath]
++++
G_{BT709} = R_{BT601_525} \times 0.017772 + G_{BT601_525} \times 0.965793 + B_{BT601_525} \times 0.016435
++++
[latexmath]
++++
B_{BT709} = R_{BT601_525} \times -0.001622 + G_{BT601_525} \times -0.004370 + B_{BT601_525} \times 1.005991
++++

[latexmath]
++++
R_{BT709} = R_{BT601_625} \times 1.044043 + G_{BT601_625} \times -0.044043
++++
[latexmath]
++++
...
[latexmath]
++++
B_{BT709} = G_{BT601_625} \times 0.011793 + B_{BT601_625} \times 0.988207
++++
...
*Parameters*
  ...
  * `[out]` _output_ - The output image to which to convert `[*REQ-0098*]`[[REQ-0098]].
    The output image must have the same dimensions as the input image.
...
Please note that in the above tables:
  ...
  * num denotes supported scalar data types are `<<VX_TYPE_INT8,VX_TYPE_INT8>>`,
    `<<VX_TYPE_UINT8,VX_TYPE_UINT8>>`, `<<VX_TYPE_INT16,VX_TYPE_INT16>>`, `<<VX_TYPE_UINT16,VX_TYPE_UINT16>>`,
    `<<VX_TYPE_INT32,VX_TYPE_INT32>>`, `<<VX_TYPE_UINT32,VX_TYPE_UINT32>>`, `<<VX_TYPE_SIZE,VX_TYPE_SIZE>>`, and
    `<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>`.
  ...
  * If the operation result cannot be stored in output data type without
...
// Brief text:
[Graph] Selects one of two data objects depending on the value of a
...
*Parameters*
  ...
  * `[out]` _output_ - The output image `[*REQ-0135*]`[[REQ-0135]].
    The output image must have the same dimensions as the input image.
  * `[in]` _policy_ - A `<<VX_TYPE_ENUM,VX_TYPE_ENUM>>` of the `<<vx_convert_policy_e>>`
...
*Parameters*
  ...
  * `[in]` _policy_ - A `<<VX_TYPE_ENUM,VX_TYPE_ENUM>>` of the `<<vx_convert_policy_e>>`
    enumeration.
  * `[in]` _shift_ - A `<<VX_TYPE_INT32,VX_TYPE_INT32>>` of the shift
    value.
...
// @details
The client can supply a `vx_int16` typed convolution matrix latexmath:[C_{m,n}].
...
.Note
====
The above equation for this function is different from an equivalent
...
*Parameters*
  ...
  * `[out]` _output_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format `[*REQ-0147*]`[[REQ-0147]].
    The output image must have the same dimensions as the input image.
...
The algorithm is based on the cumulative distribution function (CDF) of the
input histogram. Given an M×N input image, let latexmath:[H(i)] be the
histogram count for intensity value latexmath:[i \in [0, 255\]].
The CDF is:
...
Let latexmath:[CDF_{min}] be the minimum non-zero value of latexmath:[CDF].
The equalized output intensity for each input pixel latexmath:[p] is:
...
When latexmath:[M \times N = CDF_{min}] (all pixels share a single intensity
value), the output is implementation-defined.
...
*Parameters*
  ...
  * `[out]` _output_ - The grayscale output image of type
    `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` with equalized brightness and contrast `[*REQ-0164*]`[[REQ-0164]].
    The output image must have the same dimensions as the input image.
...
*Parameters*
  ...
  * `[out]` _output_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>` format `[*REQ-0170*]`[[REQ-0170]].
    The output image must have the same dimensions and same format as the input image.
...
candidate point.
If latexmath:[N] contiguous pixels are brighter than the candidate point by at
least a threshold value latexmath:[t] or darker by at least latexmath:[t], then the
...
(16 pixels) to classify whether a candidate point latexmath:[p] is actually a
...
The two conditions for FAST corner detection can be expressed as:
  ...
  * C1: A set of latexmath:[N] contiguous pixels latexmath:[S], latexmath:[\forall x in S, I_{x} > I_{p} + t]
  * C2: A set of latexmath:[N] contiguous pixels latexmath:[S], latexmath:[\forall x in S, I_{x} < I_{p} - t]

So when either of these two conditions is met, the candidate latexmath:[p] is
classified as a corner.

In this version of the FAST algorithm, the minimum number of contiguous
pixels latexmath:[N] is 9 (FAST9).
...
[latexmath]
++++
\texttt{UINT8_MIN} < t < \texttt{UINT8_MAX}
++++
...
The corner response latexmath:[C_{p}] function is defined as the largest threshold
latexmath:[t] for which the pixel latexmath:[p] remains a corner.

*Non-maximum suppression:*

If the _nonmax_suppression_ flag is true, a non-maxima suppression step is
applied on the detected corners.
The corner with coordinates latexmath:[(x,y)] is kept if and only if
...
*Parameters*
  ...
  * `[in]` _strength_thresh_ - Threshold on difference between intensity of
    ...
    (`<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` scalar), with a value in the range of latexmath:[0.0 \leq strength_thresh < 256.0] `[*REQ-0175*]`[[REQ-0175]].
...
*Parameters*
  ...
  * `[in]` _strength_thresh_ - Threshold on difference between intensity of
    the central pixel and pixels on Bresenham's circle of radius 3
    (`<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` scalar), with a value in the range of latexmath:[0.0 \leq strength_thresh < 256.0].
...
Two 1-D centered, point discrete derivative masks are applied to the input
image in the horizontal and vertical directions.
latexmath:[M_{h} = [-1, 0, 1]] and latexmath:[M_{v} = [-1, 0, 1]^{T}] latexmath:[G_{v}] is the
result of applying mask latexmath:[M_{v}] to the input image, and latexmath:[G_{h}] is the
result of applying mask latexmath:[M_{h}] to the input image.
...
[latexmath]
++++
\theta(x,y) = arctan(G_{v}(x,y), G_{h}(x,y))
++++
...
[latexmath]
++++
magnitudes(c) = \frac{1}{(cell\_width \times cell\_height)}
           \sum_{w=0}^{cell\_width} \sum_{h=0}^{cell\_height} G_c(w,h)
++++

where latexmath:[G_{c}] is the gradient magnitudes related to cell latexmath:[c].
...
These are quantized into a set of histogram bins based on the num_bins
parameter.
...
[latexmath]
++++
bins(c, n) = \sum_{w=0}^{cell\_width}
        \sum_{h=0}^{cell\_height} G_c(w,h) \times 1[B_c(w,h,num\_bins) ==
        n]
++++

where latexmath:[B_{c}] produces the histogram bin number based on the gradient
orientation of the pixel at location latexmath:[(w,h)] in cell latexmath:[c] based on
the latexmath:[num_bins] and
...
is a delta-function with value 1 when latexmath:[B_{c}(w,h,num_bins) == n] or 0
otherwise.
...
*Parameters*
  ...
  * `[out]` _magnitudes_ - [optional] The output average gradient magnitudes
    ...
    latexmath:[[floor(image_{width} / cell_{width}), floor(image_{height} / cell_{height})]] `[*REQ-0204*]`[[REQ-0204]].
    Use NULL if not needed.
  * `[out]` _bins_ - The output gradient orientation histograms per cell of
    `<<vx_tensor>>` of type `<<VX_TYPE_INT16,VX_TYPE_INT16>>` of size
    latexmath:[[floor(image_{width} / cell_{width}), floor(image_{height} / cell_{height}), num_{bins}]] `[*REQ-0205*]`[[REQ-0205]].
...
tensor are normalized by the average cell gradient magnitudes.
...
cell histogram (latexmath:[num_{bins}]).
These blocks typically overlap, meaning that each cell histogram contributes
more than once to the final descriptor.
To normalize a block its cell histograms latexmath:[h] are grouped together to
form a vector latexmath:[v = [h_{1}, h_{2}, h_{3}, \ldots, h_{n}]].
This vector is normalized using L2-Hys which means performing L2-norm on
...
where latexmath:[|| v ||_{k}] be its k-norm for latexmath:[k=1, 2],
and latexmath:[\epsilon] be a small constant.
...
[latexmath]
++++
(\lfloor (I_{w} - W_{w}) / W_{s} \rfloor + 1,
++++
[latexmath]
++++
\lfloor (I_{h} - W_{h}) / W_{s} \rfloor + 1,
++++
[latexmath]
++++
\lfloor (W_{w} - B_{w}) / B_{s} + 1 \rfloor \times \lfloor (W_{h} - B_{h}) / B_{s} + 1 \rfloor \times ( (B_{w} \times B_{h}) / (C_{w} \times C_{h}) ) \times num_{bins})
++++
...
*Parameters*
  ...
  * `[out]` _magnitudes_ - [optional] The output average gradient magnitudes per cell
    of `<<vx_tensor>>` of type `<<VX_TYPE_INT16,VX_TYPE_INT16>>` of size
    latexmath:[[floor(image_{width} / cell_{width}), floor(image_{height} / cell_{height})]].
    Use NULL if not needed.
  * `[out]` _bins_ - The output gradient orientation histograms per cell of
    `<<vx_tensor>>` of type `<<VX_TYPE_INT16,VX_TYPE_INT16>>` of size
    latexmath:[[floor(image_{width} / cell_{width}), floor(image_{height} / cell_{height}), num_{bins}]].
...
For this reason, it is not recommended that this "`immediate mode`" version
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _input_ - The input image of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>`.
  * `[in]` _magnitudes_ - [optional] The average gradient magnitudes per cell of
...
where latexmath:[V_{c}] is the thresholded corner value.
...
\frac{1}{4 \times 255 \times b} \times transpose(\mathbf{sobel}_{x}) =
\frac{1}{4 \times 255 \times b} \times
\begin{bmatrix}
  -1 & -2 & -1 \\
   0 & 0  &  0 \\
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\mathbf{Sobel}_y(Normalized)=
\frac{1}{16 \times 255 \times b} \times transpose(\mathbf{sobel}_{x})
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
--
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\mathbf{Sobel}_y(Normalized)=
\frac{1}{64 \times 255 \times b} \times transpose(\mathbf{sobel}_{x})
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
--

latexmath:[V_{c}] is then non-maximally suppressed, returning the same results as
using the following algorithm:

  * Filter the features using the non-maximum suppression algorithm defined
    for `<<vxFastCornersNode>>`.
  * Create an array of features sorted by latexmath:[V_{c}] in descending order:
    latexmath:[V_{c}(j) > V_{c}(j+1)].
  * Initialize an empty feature set latexmath:[F = {}]
  * For each feature latexmath:[j] in the sorted array, while latexmath:[V_{c}(j) > T_{c}]:
  ** If there is no feature i in latexmath:[F] such that the Euclidean distance
     between pixels i and j is less than latexmath:[r], add the feature latexmath:[j] to
     the feature set latexmath:[F].

An implementation shall support all values of Euclidean distance latexmath:[r]
that satisfy `[*REQ-0219*]`[[REQ-0219]]: latexmath:[0 \leq min\_distance \leq 30] The feature set latexmath:[F] is
...
*Parameters*
  ...
  * `[in]` _sensitivity_ - The `<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` scalar sensitivity
    threshold latexmath:[k] from the Harris-Stephens equation `[*REQ-0224*]`[[REQ-0224]].
...
*Parameters*
  ...
  * `[in]` _input_ - The input `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` image.
  * `[in]` _strength_thresh_ - The `<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` minimum threshold
    with which to eliminate Harris Corner scores (computed using the normalized
    Sobel kernel).
  * `[in]` _min_distance_ - The `<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` radial Euclidean
    distance for non-maximum suppression.
  * `[in]` _sensitivity_ - The `<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` scalar sensitivity
    threshold latexmath:[k] from the Harris-Stephens equation.
...
A pixel with intensity latexmath:[I] will result in incrementing histogram bin
latexmath:[i] where `[*REQ-0231*]`[[REQ-0231]]
...
// @details
The node implements the Progressive Probabilistic Hough Transform described
...
accumulator, to detect the existence of a line described by latexmath:[r = x cos \theta + y sin \theta].
The dimension of the accumulator equals the number of unknown parameters,
i.e., two, considering quantized values of latexmath:[r] and latexmath:[\theta] in the
pair latexmath:[(r,\theta)].
For each pixel at (x,y) and its neighborhood, the Hough transform algorithm
determines if there is enough evidence of a straight line at that pixel.
If so, it will calculate the parameters latexmath:[(r,\theta)] of that line, and
...
*Parameters*
  ...
  * `[in]` _input_ - A single channel binary source image of type
  ...
  * `[out]` _lines_array_ - lines_array contains array of lines `[*REQ-0241*]`[[REQ-0241]], see
    `<<vx_line2d_t>>` The order of lines is implementation-defined.
...
where, for latexmath:[x \geq 0] and latexmath:[y \geq 0]
...
Local binary pattern is defined as: Each pixel (y,x) generates an unsigned 8-bit value
describing the local binary pattern around the input pixel `SrcImg[y,x]`, by comparing the pixel
value with its 8 neighbors (selected neighbors of the 3x3 or 5x5 window).
...
Each pixel (y,x) generates an unsigned 8-bit value describing the modified local
binary pattern around the pixel, by comparing the average of 8 neighbor
pixels with its 8 neighbors (5x5 window) `[*REQ-0253*]`[[REQ-0253]].
...
or discontinuities (smaller than 2 or equal) in the circular binary
presentation.
...
[latexmath]
++++
U = |s(g_7 - g_c) - s(g_0 - g_c)| + \sum_{p=1}^{7} |s(g_p -
     g_c) - s(g_{p-1} - g_c)|
++++

With the above definitions, the uniform LBP equation is defined as `[*REQ-0254*]`[[REQ-0254]].
...
*Parameters*
  ...
  * `[in]` _in_ - The input image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` format `[*REQ-0256*]`[[REQ-0256]].
  ...
  * `[out]` _out_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` format `[*REQ-0259*]`[[REQ-0259]].
    The output image must have the same dimensions as the input image.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _in_ - The input image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` format.
  * `[in]` _format_ - A variation of LBP like original LBP and mLBP.
    See `<<vx_lbp_format_e>>`
  * `[in]` _kernel_size_ - Kernel size.
    Only size of 3 and 5 are supported.
  * `[out]` _out_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` format.
    The output image must have the same dimensions as the input image.
...
`<<VX_SCALE_PYRAMID_HALF,VX_SCALE_PYRAMID_HALF>>` and the number of levels equal to latexmath:[N+1],
where latexmath:[N] is the number of levels in the laplacian pyramid.
The border mode for the Gaussian pyramid calculation should be
`<<VX_BORDER_REPLICATE,VX_BORDER_REPLICATE>>`.
Then, for each latexmath:[i = 0 \ldots N-1], the Laplacian level latexmath:[L_{i}] is
computed as:
...
Here latexmath:[G_{i}] is the latexmath:[i]-th level of the Gaussian pyramid.

latexmath:[UpSample(I)] is computed by injecting even zero rows and columns and
then convolves the result with the Gaussian 5x5 filter multiplied by 4.

[latexmath]
++++
UpSample(I)_{x,y} = 4 \sum_{k=-2}^{2} \sum_{l=-2}^{2}
...
latexmath:[L_{0}] shall always have the same resolution as the input image `[*REQ-0263*]`[[REQ-0263]].
The output image is equal to latexmath:[G_{N}].
...
*Parameters*
  ...
  * `[in]` _grad_x_ - The input x image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format `[*REQ-0272*]`[[REQ-0272]].
  * `[in]` _grad_y_ - The input y image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format `[*REQ-0273*]`[[REQ-0273]].
  * `[out]` _mag_ - The output magnitude image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format `[*REQ-0274*]`[[REQ-0274]].
    The output image must have the same dimensions as the input image.
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _grad_x_ - The input x image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format.
  * `[in]` _grad_y_ - The input y image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format.
  * `[out]` _mag_ - The output magnitude image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format.
    The output image must have the same dimensions as the input image.
...
image respectively.
latexmath:[R] is the compare map.
latexmath:[T] is the template image.
latexmath:[I] is the image on which the template is searched.
...
`<<vx_comp_metric_e>>`.
The node has a limitation on the template image size (width*height).
It should not be larger than 65535.
...
*Parameters*
  ...
  * `[in]` _in2_ - The second input image `[*REQ-0290*]`[[REQ-0290]].
    Must be of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`.
  * `[out]` _out_ - The output image which will hold the result of max `[*REQ-0291*]`[[REQ-0291]].
    The output image must have the same dimensions and same format as the input image.
...
[latexmath]
++++
\mu = \frac{\left(\sum_{y=0}^h \sum_{x=0}^w src(x,y)
...
[latexmath]
++++
\sigma = \sqrt{\frac{\left(\sum_{y=0}^h \sum_{x=0}^w (\mu -
...
*Parameters*
  ...
  * `[out]` _output_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>` format `[*REQ-0303*]`[[REQ-0303]].
    The output image must have the same dimensions and same format as the input image `[*REQ-0304*]`[[REQ-0304]].
...
*Parameters*
  ...
  * `[in]` _in2_ - The second input image `[*REQ-0309*]`[[REQ-0309]].
    Must be of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>`.
  * `[out]` _out_ - The output image which will hold the result of min `[*REQ-0310*]`[[REQ-0310]].
    The output image must have the same dimensions and same format as the input image.
...
*Parameters*
  ...
  * `[out]` _output_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>` format `[*REQ-0331*]`[[REQ-0331]],
    which must have the same dimensions and same format as the input image `[*REQ-0332*]`[[REQ-0332]].
...
A pixel with coordinates latexmath:[(x,y)] is kept if and only if it is greater
than or equal to its top left neighbors; and greater than its bottom right
neighbors `[*REQ-0337*]`[[REQ-0337]].
For example, for a window size of 3, latexmath:[P(x,y)] is retained if the
...
*Parameters*
  ...
  * `[in]` _mask_ - [optional] Restrict suppression to a ROI.
  ...
  * `[in]` _win_size_ - The size of window over which to perform the
    ...
    input image.
  * `[out]` _output_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format `[*REQ-0342*]`[[REQ-0342]].
    The output image must have the same dimensions and same format as the input image.
...
latexmath:[V] for each point in the old image tracking points array, using the
following equation:
...
Where latexmath:[I_{x}] and latexmath:[I_{y}] are obtained using the Scharr gradients on
the input image:
...
latexmath:[I_{t}] is obtained by a simple difference between the same pixel in both
images.
latexmath:[I] is defined as the adjacent pixels to the point latexmath:[p(x,y)] under
consideration.
With a given window size of latexmath:[M], latexmath:[I] is latexmath:[M^{2}] points.
The pixel latexmath:[p(x,y)] is centered in the window.
...
latexmath:[I_{t}] to be the difference between the old image and the pixel with the
...
(The determinant of the matrix is less than a threshold : latexmath:[10^{-7}].) Or
the minimum eigenvalue of the matrix is smaller than a threshold
(latexmath:[10^{-4}]).
...
algorithm starts by calculating latexmath:[I_{t}] as the difference between the old
...
*Parameters*
  ...
  * `[in]` _new_points_estimates_ - An array of estimates on what is the
...
[latexmath]
++++
\phi = \text{atan2}(grad_y(x,y),\, grad_x(x,y))
++++

Where latexmath:[\phi] is then translated to latexmath:[0 \leq \phi < 2 \pi].
Each latexmath:[\phi] value is then mapped to the range 0 to 255 inclusive `[*REQ-0364*]`[[REQ-0364]].
...
*Parameters*
  ...
  * `[in]` _grad_x_ - The input x image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format `[*REQ-0366*]`[[REQ-0366]].
  * `[in]` _grad_y_ - The input y image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format `[*REQ-0367*]`[[REQ-0367]].
  * `[out]` _orientation_ - The output phase image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` format `[*REQ-0368*]`[[REQ-0368]].
    The output image must have the same dimensions as the input image.
...
*Parameters*
  ...
  * `[in]` _grad_x_ - The input x image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format.
  * `[in]` _grad_y_ - The input y image in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format.
  * `[out]` _orientation_ - The output phase image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` format.
    The output image must have the same dimensions as the input image.
...
The scale with a value of latexmath:[1 / 2^{n}], where latexmath:[n] is an integer and
latexmath:[0 \leq n \leq 15], and 1/255 (0x1.010102p-8 C99 float hex) must be
supported.
...
[latexmath]
++++
reference(x,y,scale) = truncate( ( (int32_t)in_{1}(x,y)) \times ( (int32_t)in_{2}(x,y)) \times (double)scale)
++++
...
[latexmath]
++++
reference(x,y,scale) = round_to_nearest_even( ( (int32_t)in_{1}(x,y)) \times ( (int32_t)in_{2}(x,y)) \times (double)scale)
++++
...
[latexmath]
++++
out(x,y) = in_{1}(x,y) \times in_{2}(x,y) \times scale
++++
...
For the definition of the latexmath:[UpSample] function please see
`<<vxLaplacianPyramidNode>>`.
Correspondingly, for each pyramid level latexmath:[i = 0 \ldots N-2]:
...
for every latexmath:[(x,y)] in the destination image
...
*Parameters*
  ...
  * `[in]` _table_ - The remap table object `[*REQ-0395*]`[[REQ-0395]].
    Must be of the same dimensions as the remap table's source image.
  ...
  * `[out]` _output_ - The output `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` image `[*REQ-0397*]`[[REQ-0397]].
    Must be of the same dimensions as the remap table's destination image.
...
++++
  ...
  * For `<<VX_INTERPOLATION_NEAREST_NEIGHBOR,VX_INTERPOLATION_NEAREST_NEIGHBOR>>`, the output value is that of
    the pixel whose center is closest to the sample point `[*REQ-0403*]`[[REQ-0403]].
...
[latexmath]
++++
output(x_{input},y_{input}) = (1-s)(1-t) \times input(x_{lower},y_{lower}) + s(1-t) \times input(x_{lower}+1,y_{lower}) + (1-s)t \times input(x_{lower},y_{lower}+1) + s \times t \times input(x_{lower}+1,y_{lower}+1)
++++
--
...
As the pixel center closest to the sample position is white in all cases,
...
*Parameters*
  ...
  * `[out]` _dst_ - The destination image of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>`.
    The output image must have the same format as the input image.
  * `[in]` _type_ - The interpolation type to use. *See also:* `<<vx_interpolation_type_e>>`.
...
*Parameters*
  ...
  * `[out]` _dst_ - The destination image of type `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>`.
    The output image must have the same format as the input image.
  * `[in]` _type_ - The interpolation type. *See also:* `<<vx_interpolation_type_e>>`.
...
plane.
The Sobel Operators latexmath:[G_{x}, G_{y}] are defined as `[*REQ-0415*]`[[REQ-0415]]:
...
*Parameters*
  ...
  * `[out]` _output_x_ - [optional] The output gradient in the x direction
    ...
    The `output_x` image must have the same dimensions as the input image.
    Use NULL if not needed.
  * `[out]` _output_y_ - [optional] The output gradient in the y direction
    in `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` `[*REQ-0419*]`[[REQ-0419]].
    The `output_y` image must have the same dimensions as the input image.
    Use NULL if not needed.
...
*Parameters*
  ...
  * `[out]` _output_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or
    `<<VX_DF_IMAGE_S16,VX_DF_IMAGE_S16>>` format `[*REQ-0426*]`[[REQ-0426]].
    The output image must have the same dimensions as the input image.
...
*Return Values*
  ...
  * `<<vx_node>>` - A node reference `[*REQ-0434*]`[[REQ-0434]].
    Any possible errors preventing a successful creation should be checked
    using `<<vxGetStatus>>`
...
point positions of the input and output respectively.
...
*Parameters*
  ...
  * `[out]` _output_ - The output tensor `[*REQ-0441*]`[[REQ-0441]].
    Implementations must support output tensor data type `<<VX_TYPE_INT16,VX_TYPE_INT16>>`
    with fixed_point_position 8, and tensor data types `<<VX_TYPE_UINT8,VX_TYPE_UINT8>>`
    and `<<VX_TYPE_INT8,VX_TYPE_INT8>>`, with fixed_point_position 0.
...
*Parameters*
  ...
  * `[in]` _input3_ - [optional] The third 2D tensor `[*REQ-0448*]`[[REQ-0448]].
    Must be in the same data type as _input1_.
    Use NULL if not needed.
...
*Return Values*
  ...
  * `<<vx_node>>` - A node reference `[*REQ-0451*]`[[REQ-0451]].
    Any possible errors preventing a successful creation should be checked
    using `<<vxGetStatus>>`
...
*Parameters*
  ...
  * `[in]` _input2_ - The second 2D tensor.
    Must be in the same data type as _input1_.
  * `[in]` _input3_ - [optional] The third 2D tensor.
    Must be in the same data type as _input1_.
    Use NULL if not needed.
  * `[in]` _matrix_multiply_params_ - Matrix multiply parameters, see
    `<<vx_tensor_matrix_multiply_params_t>>`.
...
The scale with a value of latexmath:[1 / 2^{n}], where n is an integer and latexmath:[0 \leq n \leq 15], and 1/255 (0x1.010102p-8 C99 float hex) must be
supported.
The support for other values of scale is not prohibited.
Furthermore, for scale with a value of 1/255 the rounding policy of
`<<VX_ROUND_POLICY_TO_NEAREST_EVEN,VX_ROUND_POLICY_TO_NEAREST_EVEN>>` must be supported `[*REQ-0453*]`[[REQ-0453]] whereas for the
scale with value of latexmath:[1 / 2^{n}] the rounding policy of
...
*Return Values*
  ...
  * `<<vx_node>>` - A node reference `[*REQ-0463*]`[[REQ-0463]].
    Any possible errors preventing a successful creation should be checked
    using `<<vxGetStatus>>`
...
*Return Values*
  ...
  * `<<vx_node>>` - A node reference `[*REQ-0470*]`[[REQ-0470]].
    Any possible errors preventing a successful creation should be checked
    using `<<vxGetStatus>>`
...
*Return Values*
  ...
  * `<<vx_node>>` - A node reference `[*REQ-0489*]`[[REQ-0489]].
    Any possible errors preventing a successful creation should be checked
    using `<<vxGetStatus>>`
...
*Parameters*
  ...
  * `[in]` _thresh_ - The thresholding object that defines the parameters of
    ...
    output image format.
  * `[out]` _output_ - The output image in `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` or `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>` format, that will contain as pixel values
...
// @details
This kernel performs an affine transform with a 2x3 Matrix latexmath:[M] with this
method of pixel coordinate translation `[*REQ-0498*]`[[REQ-0498]].
The matrix is stored in row-major order, so element latexmath:[M_{r,c}] maps to
the C array index `matrix[r-1][c-1]` (or the equivalent flat index
`(r-1)*3 + (c-1)` for a 2×3 matrix laid out as `float[6]`):
...
// @details
This kernel performs an perspective transform with a 3x3 Matrix latexmath:[M] with
this method of pixel coordinate translation `[*REQ-0508*]`[[REQ-0508]].
The matrix is stored in row-major order, so element latexmath:[M_{r,c}] maps to
the C array index `matrix[r-1][c-1]` (or the equivalent flat index
`(r-1)*3 + (c-1)` for a 3×3 matrix laid out as `float[9]`):
...
Where latexmath:[0 \leq \alpha \leq 1].
...
*Parameters*
  ...
  * `[in]` _alpha_ - The input `<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` scalar value with a
    value in the range of latexmath:[0.0 \leq \alpha \leq 1.0] `[*REQ-0522*]`[[REQ-0522]].
...
*Parameters*
  ...
  * `[in]` _context_ - The reference to the overall context.
  * `[in]` _img1_ - The first `<<VX_DF_IMAGE_U8,VX_DF_IMAGE_U8>>` image.
  * `[in]` _alpha_ - A `<<VX_TYPE_FLOAT32,VX_TYPE_FLOAT32>>` type, the input value with the
    range latexmath:[0.0 \leq \alpha \leq 1.0].
...
// Brief text:
An object's attribute ID is within the range of latexmath:[[0,2^{8} - 1]]
(inclusive).
...
// Brief text:
A generic enumeration list can have values between latexmath:[[0,2^{12} - 1]]
(inclusive).
...
The valid range is between latexmath:[[0,2^{8} - 1]] (inclusive).
...
// Brief text:
An individual kernel in a library has its own unique ID within latexmath:[[0,2^{12} - 1]] (inclusive).
...
**Value:** `(((vx_uint32)(e) & VX_LIBRARY_MASK) >> 12)`
...
The range is latexmath:[[0,2^{8} - 1]] inclusive.
...
include::api/defines/VX_VERSION_1_3.txt[]

// end::refpage:VX_VERSION_1_3[]
...
version (e.g., 1.3.0 vs 1.3.1 vs 1.3.2).
The major and minor version are encoded in `<<VX_VERSION,VX_VERSION>>`;
this macro provides the third component.

include::api/defines/VX_VERSION_PATCH.txt[]

// end::refpage:VX_VERSION_PATCH[]

== Typedefs

// tag::refpage:vx_bitfield[]
// [desc='',type='basetypes']
=== vx_bitfield
...
include::api/basetypes/vx_bitfield.txt[]

// end::refpage:vx_bitfield[]
...
include::api/basetypes/vx_float16.txt[]

// end::refpage:vx_float16[]
...
*Enumerator*
  ...
  * [[VX_DF_IMAGE_RGBA]] `VX_DF_IMAGE_RGBA` - A single plane of 32-bit pixel
    as 4 interleaved 8-bit units of R then G then B then Alpha data.
    This uses the BT709 full range by default.
    _(Added in OpenVX 1.3.2)_
  ...
  * [[VX_DF_IMAGE_U1]] `VX_DF_IMAGE_U1` - A single plane of unsigned 1-bit
    ...
    generated.
    See `<<vx_imagepatch_addressing_t>>` for more details.
  ...
  * [[VX_DF_IMAGE_U32]] `VX_DF_IMAGE_U32` - A single plane of unsigned
    ...
    The range of data is not specified, as it may be extracted from a YUV or
    generated.
  * [[VX_DF_IMAGE_S32]] `VX_DF_IMAGE_S32` - A single plane of signed
    32-bit data.
    The range of data is not specified, as it may be extracted from a YUV or
    generated.
...
*Enumerator*
  ...
  * [[VX_ERROR_GRAPH_NOT_VERIFIED]] `VX_ERROR_GRAPH_NOT_VERIFIED` - Indicates that the graph has not yet been verified.
    Returned by APIs that require a verified graph.
    _(Added in OpenVX 1.3.2)_
  * [[VX_ERROR_TIMEOUT]] `VX_ERROR_TIMEOUT` - Indicates that an operation
    did not complete within an expected duration.
    Returned by APIs that support timeout-based execution when a timeout is configured.
    _(Added in OpenVX 1.3.2)_
  ...
  * [[VX_ERROR_OPTIMIZED_AWAY]] `VX_ERROR_OPTIMIZED_AWAY` - Indicates that
    the object referred to has been optimized out of existence.
...
--
  * [[VX_ERROR_NO_RESOURCES]] `VX_ERROR_NO_RESOURCES` - Indicates that an
    internal or implicit resource cannot be acquired (not memory).
...
--
  * [[VX_ERROR_NOT_SUPPORTED]] `VX_ERROR_NOT_SUPPORTED` - Indicates that the
    ...
    supported.
    Refer to the supplied documentation on the configured kernels. This is also
    returned if a function to set an attribute is called on a Read-only attribute.
+
--
// Detailed text
...
*Enumerator*
  ...
  * [[VX_TYPE_FLOAT16]] `VX_TYPE_FLOAT16` - A `<<vx_float16>>`.
  ...
  * [[VX_TYPE_VENDOR_OBJECT_START]] `VX_TYPE_VENDOR_OBJECT_START` - A
    vendor-defined object base index.
  ...
  * [[VX_TYPE_SCALAR]] `VX_TYPE_SCALAR` - A `<<vx_scalar>>`.
    Used when a completely generic type is needed for kernel validation.
...
*Enumerator*
  ...
  * [[VX_ID_RENESAS]] `VX_ID_RENESAS` - Renesas Electronics.
  ...
  * [[VX_ID_BOSCH]] `VX_ID_BOSCH`- Robert Bosch GmbH.
...
*Return Values*
  ...
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - ref is not a valid `<<vx_reference>>`
    reference.
  * `<<VX_ERROR_INVALID_PARAMETERS,VX_ERROR_INVALID_PARAMETERS>>` - If any of the other parameters are incorrect.
  * `<<VX_ERROR_NOT_SUPPORTED,VX_ERROR_NOT_SUPPORTED>>` - If the attribute is not supported on this implementation.
...
the *User* intent.
For example: these can be interpreted as hints to the system about cache
operations or marshaling operations.
...
*Enumerator*
  ...
  * [[VX_CONTEXT_IMPLEMENTATION]] `VX_CONTEXT_IMPLEMENTATION` - Queries the
    context for its implementation name `[*REQ-0550*]`[[REQ-0550]].
  ...
  * [[VX_CONTEXT_OPTICAL_FLOW_MAX_WINDOW_DIMENSION]]
    ...
    dimension of the <<Optical Flow Pyramid (LK)>> kernel `[*REQ-0561*]`[[REQ-0561]].
    The value of this attribute shall be equal to or greater than '9' `[*REQ-0562*]`[[REQ-0562]].
    Read-only `[*REQ-1986*]`[[REQ-1986]].
    Use a `<<vx_size>>` parameter.
+
--
// Detailed text
*See also:* `<<vxOpticalFlowPyrLKNode,vxOpticalFlowPyrLKNode>>`.
--
...
--
  * [[VX_CONTEXT_UNIQUE_KERNEL_TABLE]] `VX_CONTEXT_UNIQUE_KERNEL_TABLE` -
    Returns the table of all the unique kernels that exist in the context `[*REQ-0563*]`[[REQ-0563]].
...
Read-only `[*REQ-1989*]`[[REQ-1989]].
Use a `<<vx_size>>` parameter.
--
  * [[VX_CONTEXT_MAX_TENSOR_DIMS]] `VX_CONTEXT_MAX_TENSOR_DIMS` - The
    maximum number of tensor dimensions supported by the implementation `[*REQ-0566*]`[[REQ-0566]].
    Read-only.
    Use a `<<vx_size>>` parameter.
...
*Enumerator*
  ...
  * [[VX_TERM_CRITERIA_EPSILON]] `VX_TERM_CRITERIA_EPSILON` - Indicates a
    termination after matching against the value of epsilon provided to the
    function `[*REQ-0572*]`[[REQ-0572]].
  * [[VX_TERM_CRITERIA_BOTH]] `VX_TERM_CRITERIA_BOTH` - Indicates that both
    an iterations and epsilon method are employed `[*REQ-0573*]`[[REQ-0573]].
...
*Return Values*
  ...
  * `<<VX_SUCCESS,VX_SUCCESS>>` - No errors; any other value indicates
    failure.
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - context is not a valid
    `<<vx_context>>` reference.
...
See <<sub_graphs_rules>>.
The figure below shows the Graph state transition diagram.
...
--
  * [[VX_GRAPH_NUMPARAMETERS]] `VX_GRAPH_NUMPARAMETERS` - Returns the number
    ...
    Read-only `[*REQ-1992*]`[[REQ-1992]].
    Use a `<<vx_uint32>>` parameter.
  * [[VX_GRAPH_STATE]] `VX_GRAPH_STATE` - Returns the state of the graph.
    Read-only `[*REQ-1993*]`[[REQ-1993]].
    Use a `<<vx_enum>>` parameter. See `<<vx_graph_state_e>>`.
...
After the graph verifies successfully then processing occurs `[*REQ-0606*]`[[REQ-0606]].
...
After the graph verifies successfully then processing occurs `[*REQ-0635*]`[[REQ-0635]].
If the graph was previously verified via `<<vxVerifyGraph>>` or
...
// Detailed text:
*Precondition:* Memory for data objects is not guaranteed to exist before
this call.
...
--
  * [[VX_NODE_BORDER]] `VX_NODE_BORDER` - Gets or sets the border mode of
    ...
    Use a `<<vx_border_t>>` parameter with a default value of
    `<<VX_BORDER_UNDEFINED,VX_BORDER_UNDEFINED>>`.
...
*Return Values*
  ...
  * `<<VX_SUCCESS,VX_SUCCESS>>` - No errors; any other value indicates failure `[*REQ-0713*]`[[REQ-0713]].
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - node is not a valid `<<vx_node>>`
    reference.
...
*Parameters*
  ...
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the memory type of the memory referenced by the user_ptr `[*REQ-0739*]`[[REQ-0739]].
...
*Return Values*
  ...
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - array is not a valid `<<vx_array>>`
    reference.
  * `<<VX_ERROR_INVALID_PARAMETERS,VX_ERROR_INVALID_PARAMETERS>>` - Another parameter is incorrect.
...
*Returns:* An array reference `<<vx_array>>` `[*REQ-0751*]`[[REQ-0751]].
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in,out]` _arr_ - The reference to the Array `[*REQ-0776*]`[[REQ-0776]].
...
*Return Values*
  ...
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - arr is not a valid `<<vx_array>>`
    reference.
  * `<<VX_ERROR_INVALID_PARAMETERS,VX_ERROR_INVALID_PARAMETERS>>` - The _new_num_items_ is greater than the
...
*Parameters*
  ...
  * `[in]` _map_id_ - The unique map identifier that was returned when
...
*Parameters*
  ...
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the memory type of the memory referenced by the user_ptr `[*REQ-0800*]`[[REQ-0800]].
...
*Parameters*
  ...
  * `[in]` _conv_ - The convolution matrix object to query `[*REQ-0818*]`[[REQ-0818]].
...
*Parameters*
  ...
  * `[in]` _conv_ - The convolution object to set `[*REQ-0829*]`[[REQ-0829]].
...
*Parameters*
  ...
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the memory type of the memory referenced by the user_ptr `[*REQ-0856*]`[[REQ-0856]].
...
latexmath:[[offset, offset + range - 1]] defined by a start _offset_ and valid _range_,
...
*Parameters*
  ...
  * `[in]` _offset_ - The start offset into the range value that marks the
    beginning of the 1D Distribution `[*REQ-0862*]`[[REQ-0862]].
...
*Parameters*
  ...
  * `[in]` _offset_ - The start offset into the range value that marks the
    beginning of the 1D Distribution `[*REQ-0868*]`[[REQ-0868]].
  * `[in]` _range_ - The total number of the consecutive values of the
...
*Return Values*
  ...
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - distribution is not a valid `<<vx_distribution>>` reference.
  * `<<VX_ERROR_INVALID_PARAMETERS,VX_ERROR_INVALID_PARAMETERS>>` - Another parameter is incorrect.
  * `<<VX_ERROR_NO_MEMORY,VX_ERROR_NO_MEMORY>>` - Internal memory allocation failed.
...
====
  ...
  * scale_x, scale_y - The relationship of scaling from the primary plane
    ...
    An integer down-scaling factor of latexmath:[f] shall be set to a value equal
    to latexmath:[scale = unity / f] and an integer up-scaling factor of latexmath:[f]
    shall be set to a value of latexmath:[scale = unity \times f].
    latexmath:[unity] is defined as `<<VX_SCALE_UNITY,VX_SCALE_UNITY>>`.
...
corner, i.e. origin. A `<<VX_DF_IMAGE_U1,VX_DF_IMAGE_U1>>` image always starts on a byte boundary and each row has a
_stride_y_ that is a multiple of whole bytes, which means padding bits of implementation-defined values may be present at the end of
each row. Image patches can only be accessed at a multiple of eight pixels: the x-coordinate must be a multiple of eight.
...
// Detailed text:
  * RGB[3] - `<<VX_DF_IMAGE_RGB,VX_DF_IMAGE_RGB>>` format in the R,G,B order
  * RGBA[4] - `<<VX_DF_IMAGE_RGBA,VX_DF_IMAGE_RGBA>>` format in the R,G,B,A order
  * RGBX[4] - `<<VX_DF_IMAGE_RGBX,VX_DF_IMAGE_RGBX>>` format in the R,G,B,X order
...
*Enumerator*
  ...
  * [[VX_COLOR_SPACE_BT601_625]] `VX_COLOR_SPACE_BT601_625` - Use to
    indicate that the BT.601 coefficients and EBU primaries are used for
    conversions.
...
*Enumerator*
  ...
  * [[VX_IMAGE_UNIFORM_VALUE]] `VX_IMAGE_UNIFORM_VALUE` - Queries the image
    uniform value if any. If `<<vx_image>>` was not created using vxCreateUniformImage,
    `<<VX_ERROR_NOT_SUPPORTED>>` is returned `[*REQ-0918*]`[[REQ-0918]].
...
*Parameters*
  ...
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the memory type of the memory referenced by the user_ptr `[*REQ-0934*]`[[REQ-0934]].
...
Updates to this image update the parent image and vice versa.
...
and instead _stride_y_ {geq} latexmath:[\lceil dim\_x / 8 \rceil] `[*REQ-0964*]`[[REQ-0964]].
====
...
Updates to this image update the parent image.
...
in the parent image, i.e., _start_x_ in the `<<vx_rectangle_t>>` must be a
...
*Parameters*
  ...
  ** _new_ptrs_ is non-`NULL`.
     _new_ptrs_[i] must be non-`NULL` for each i such as 0 <= i < num_planes,
  ...
  ** _prev_ptrs_ is non-`NULL`.
     ...
     planes.
     For each i such as 0 <= i < num_planes, _prev_ptrs_[i] is set to the
...
*Parameters*
  ...
  * `[in]` _map_id_ - The unique map identifier that was returned by
...
*Parameters*
  ...
  * `[in]` _user_ptr_ - The address of the memory location where to store
    ...
    In the user memory, the LUT is represented as an array with elements of
  ...
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the memory type of the memory referenced by the user_ptr `[*REQ-1084*]`[[REQ-1084]].
...
*Parameters*
  ...
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the memory type of the memory referenced by the user_ptr `[*REQ-1147*]`[[REQ-1147]].
...
*Parameters*
  ...
  * `[in]` _data_type_ - The vx_type_e that represents the data type of the matrix data elements `[*REQ-1152*]`[[REQ-1152]].
  * `[in]` _columns_ - The first dimensionality `[*REQ-1154*]`[[REQ-1154]].
  * `[in]` _rows_ - The second dimensionality `[*REQ-1155*]`[[REQ-1155]].

*Returns:* A matrix reference `<<vx_matrix>>` `[*REQ-1156*]`[[REQ-1156]].
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
+
--
latexmath:[(r-R/2 + 0.5)^{2} / (R/2)^{2} + (c-C/2 + 0.5)^{2} / (C/2)^{2}] is less than
...
*Parameters*
  ...
  * `[in]` _data_type_ - The vx_type_e that represents the data type of the matrix data elements `[*REQ-1180*]`[[REQ-1180]].
...
*Returns:* A matrix reference `<<vx_matrix>>` `[*REQ-1184*]`[[REQ-1184]].
Any possible errors preventing a successful creation should be checked using
`<<vxGetStatus>>`.
...
*Parameters*
  ...
  * `[in]` _mat_ - The matrix object to query `[*REQ-1186*]`[[REQ-1186]].
...
with a width of latexmath:[5 = \mathbf{ceil}(9 \times 0.5)] and a level two image with a
width of latexmath:[3 = \mathbf{ceil}(5 \times 0.5)].
Position latexmath:[(r_{N},c_{N})] at level latexmath:[N] corresponds to position
latexmath:[(r_{N-1} / \mathbf{scale}, c_{N-1} / \mathbf{scale})]
at level latexmath:[N-1].
...
// Brief text:
Use to indicate an ORB scaled pyramid whose scaling factor is
...
*Parameters*
  ...
  * `[in]` _user_ptr_ - The address of the user memory location where to
    ...
    _user_ptr_ is the address of the top-left element of the remap
  ...
  ** `<<VX_READ_ONLY,VX_READ_ONLY>>` means that data is copied from the remap object into
     the user memory pointed to by _user_ptr_ `[*REQ-1263*]`[[REQ-1263]].
  ...
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the type of the memory pointed to by _user_ptr_ `[*REQ-1265*]`[[REQ-1265]].
...
*Parameters*
  ...
  * `[in]` _src_width_ - Width of the source image in pixels `[*REQ-1270*]`[[REQ-1270]].
...
*Parameters*
  ...
  * `[in]` _src_width_ - Width of the source image in pixels `[*REQ-1277*]`[[REQ-1277]].
...
*Parameters*
  ...
  * `[out]` _ptr_ - The address of a pointer where the function returns
    where (\*_ptr_) is the address of the top-left element of the remap
...
*Parameters*
  ...
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the memory type of the memory referenced by the user_ptr `[*REQ-1341*]`[[REQ-1341]].
...
*Parameters*
  ...
  * `[in]` _user_mem_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the memory type of the memory referenced by the user_ptr `[*REQ-1353*]`[[REQ-1353]].
...
*Parameters*
  ...
  * `[in]` _data_type_ - The type of data to hold `[*REQ-1358*]`[[REQ-1358]].
    ...
    `<<VX_TYPE_VENDOR_STRUCT_END,VX_TYPE_VENDOR_STRUCT_END>>`.
    Or must be a `<<vx_enum>>` returned from `<<vxRegisterUserStruct>>`.
  * `[in]` _ptr_ - The pointer to the initial value of the scalar or NULL. If NULL,
    the initial value of the scalar, if any, is implementation-defined `[*REQ-1359*]`[[REQ-1359]].
...
*Parameters*
  ...
  * `[in,out]` _true_value_ptr_ - The address of the memory location where to
  ...
  * `[in,out]` _false_value_ptr_ - The address of the memory location where
...
*Parameters*
  ...
  * `[in,out]` _lower_value_ptr_ - The address of the memory location where
  ...
  * `[in,out]` _upper_value_ptr_ - The address of the memory location where
  ...
  ** `<<VX_READ_ONLY,VX_READ_ONLY>>` means that thresholding values are copied from the
     ...
     (*_upper_value_ptr_) unions that corresponds to the input image format of
...
*Parameters*
  ...
  * `[in,out]` _value_ptr_ - The address of the memory location where to
...
following `[*REQ-1419*]`[[REQ-1419]]:
  ...
  * `<<VX_DF_IMAGE_RGBA,VX_DF_IMAGE_RGBA>>` : false={0, 0, 0, 0}, true={255,255,255,255}
...
*Parameters*
  ...
  * `[in]` _thresh_ - The threshold object to query `[*REQ-1432*]`[[REQ-1432]].
...
*Parameters* 
  ...
  * `[in]` _exemplar_ - The exemplar object that defines the type of object
    ...
    Only exemplar types of `<<vx_object_array>>` and `<<vx_convolution>>` are not allowed.
...
// Brief text:
Queries an attribute from the ObjectArray `[*REQ-1474*]`[[REQ-1474]].
...
*Enumerator*
  ...
  * [[VX_TENSOR_NUMBER_OF_DIMS]] `VX_TENSOR_NUMBER_OF_DIMS` - Number of
    dimensions. Read-only. Use a `<<vx_size>>` parameter `[*REQ-1491*]`[[REQ-1491]].
  * [[VX_TENSOR_DIMS]] `VX_TENSOR_DIMS` - Dimension sizes. Read-only. Use a `<<vx_size>>` * parameter `[*REQ-1492*]`[[REQ-1492]].
  * [[VX_TENSOR_DATA_TYPE]] `VX_TENSOR_DATA_TYPE` - Tensor data element data
    type. Read-only. Use a `<<vx_enum>>` parameter `[*REQ-1493*]`[[REQ-1493]].
    `<<vx_type_e>>`
  * [[VX_TENSOR_FIXED_POINT_POSITION]] `VX_TENSOR_FIXED_POINT_POSITION` -
    Fixed point position when the input element type is integer. Read-only. Use a `<<vx_int8>>` parameter `[*REQ-1494*]`[[REQ-1494]].
...
// Brief text:
Allows the application to copy a view patch from/into a tensor object `[*REQ-1495*]`[[REQ-1495]].
...
*Parameters*
  ...
  * `[in]` _user_ptr_ - The address of the memory location where to store
    ...
    latexmath:[accessible memory in bytes \geq (end[last_dimension] - start[last_dimension]) * stride[last_dimension]].
+
The layout of the user memory must follow a row major order `[*REQ-1504*]`[[REQ-1504]].
  ...
  * `[in]` _user_memory_type_ - A `<<vx_memory_type_e>>` enumeration that
    specifies the memory type of the memory referenced by the user_ptr `[*REQ-1508*]`[[REQ-1508]].
...
This has to be a portion of the third lowest dimension, and the stride
corresponds to that third dimension.
...
// Brief text:
Creates a reference to a tensor object that was externally allocated `[*REQ-1525*]`[[REQ-1525]].
...
Updates to this tensor data update the parent tensor data.
...
*Parameters*
  ...
  * `[in]` _tensor_ - The reference to the tensor object that is the source
    or the destination for direct access `[*REQ-1553*]`[[REQ-1553]].
  * `[in]` _number_of_dims_ - The number of dimensions `[*REQ-1554*]`[[REQ-1554]]. Must be same as tensor number_of_dims.
    Error return if 0 or greater than number of tensor dimensions.
    If smaller than number of tensor dimensions, the lower dimensions are
    assumed.
  * `[in]` _view_start_ - Array of patch start points in each dimension `[*REQ-1555*]`[[REQ-1555]]. This is an optional parameter and will be zero when NULL.
  * `[in]` _view_end_ - Array of patch end points in each dimension `[*REQ-1556*]`[[REQ-1556]]. This is an optional parameter and will be dims[] of tensor when NULL.
  * `[out]` _map_id_ - The address of a `<<vx_map_id>>` variable where the
    function returns a map identifier `[*REQ-1557*]`[[REQ-1557]].
  ** (\*map_id) must eventually be provided as the map_id parameter of a call
     to `<<vxUnmapTensorPatch>>`.
  ...
  * `[out]` _ptr_ - The address of a pointer that the function sets to the
                   address where the requested data can be accessed `[*REQ-1560*]`[[REQ-1560]]. The returned (\*_ptr_) address
  ...
  * `[in]` _usage_ - This declares the access mode for the tensor patch `[*REQ-1561*]`[[REQ-1561]], using
                     ...
                     ** `<<VX_READ_ONLY,VX_READ_ONLY>>`: after the function call, the content of the memory location
                        pointed by (\*_ptr_) contains the tensor patch data `[*REQ-1562*]`[[REQ-1562]]. Writing into this memory location
                        is forbidden and its behavior is implementation-defined.
                     ** `<<VX_READ_AND_WRITE,VX_READ_AND_WRITE>>`: after the function call, the content of the memory
                        location pointed by (\*_ptr_) contains the tensor patch data `[*REQ-1563*]`[[REQ-1563]]; writing into this memory
                        ...
                        behavior is implementation-defined.
                    ** `<<VX_WRITE_ONLY,VX_WRITE_ONLY>>`: after the function call, values at the memory location pointed by (\*_ptr_)
...
// Brief text:
Swaps the tensor handle of a tensor previously created from handle `[*REQ-1567*]`[[REQ-1567]].
...
*Parameters*
  ...
  * `[in]` _tensor_ The reference to a tensor created from handle `[*REQ-1568*]`[[REQ-1568]].
...
*Return Values*
  ...
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - tensor is not a valid `<<vx_tensor>>` reference.
  * `<<VX_ERROR_INVALID_PARAMETERS,VX_ERROR_INVALID_PARAMETERS>>` - If any of the other parameters are
    incorrect.
  * `<<VX_ERROR_NOT_SUPPORTED,VX_ERROR_NOT_SUPPORTED>>` - If the attribute is not supported on this implementation.
...
*Return Values*
  ...
  * `<<VX_SUCCESS,VX_SUCCESS>>` - No errors; any other value indicates failure `[*REQ-1589*]`[[REQ-1589]].
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - tensor is not a valid `<<vx_tensor>>` reference.
...
// tag::refpage:vxGetUserStructEnumByName[]
// [desc='',type='protos']
==== vxGetUserStructEnumByName
...
*Precondition:* `<<vxRegisterUserStructWithName>>` should be called for this user struct.
// end::refpage:vxGetUserStructEnumByName[]
...
forth until index latexmath:[-count + 1] `[*REQ-1631*]`[[REQ-1631]].
The data originally at index latexmath:[-count + 1] move to index 0 `[*REQ-1632*]`[[REQ-1632]].
Here latexmath:[count] is the number of slots in delay ring `[*REQ-1633*]`[[REQ-1633]].
...
.Note
====
The delay index is in the range latexmath:[[-count + 1, 0]] `[*REQ-1648*]`[[REQ-1648]].
...
--
  * vx_char name[VX_MAX_KERNEL_NAME] - The kernel name in dotted hierarchical format. e.g.
...
NOTE: `<<VX_KERNEL_SWAP,VX_KERNEL_SWAP>>` and `<<VX_KERNEL_MOVE,VX_KERNEL_MOVE>>`
are extension-only enumerants from the Swap/Move extension. They are listed in
the shared kernel enumeration space but are not part of core OpenVX 1.3.2.
...
--
  * [[VX_KERNEL_WEIGHTED_AVERAGE]] `VX_KERNEL_WEIGHTED_AVERAGE` - The
    weighted average kernel `[*REQ-1694*]`[[REQ-1694]].
+
--
// Detailed text
...
*Enumerator*
  ...
  * [[VX_PARAMETER_STATE_OPTIONAL]] `VX_PARAMETER_STATE_OPTIONAL` - The
    ...
    The kernel takes care not to dereference optional parameters until it is
...
*Enumerator*
  ...
  * [[VX_HINT_PERFORMANCE_DEFAULT]] `VX_HINT_PERFORMANCE_DEFAULT` -
    Indicates to the implementation that the user does not have any specific
...
*Parameters*
  ...
  * `[in,out]` _node_ - The handle to the node that is being validated `[*REQ-1862*]`[[REQ-1862]].
...
*Parameters*
  ...
  * `[in]` _name_ - The string to use to match the kernel. The length of the string
    shall be lower than VX_MAX_KERNEL_NAME bytes `[*REQ-1899*]`[[REQ-1899]].
...
*Return Values*
  ...
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - context is not a valid
    `<<vx_context>>` reference.
  * `<<VX_ERROR_NO_RESOURCES,VX_ERROR_NO_RESOURCES>>` - The enumerations have been exhausted.
...
// Detailed text:
The module must be registered by `<<vxRegisterKernelLibrary,vxRegisterKernelLibrary>>` if it is not
a dynamic library or the module must be a dynamic library with by convention, two exported functions
named `<<vxPublishKernels,vxPublishKernels>>` and `<<vxUnpublishKernels,vxUnpublishKernels>>` `[*REQ-1923*]`[[REQ-1923]].
...
*Return Values*
  ...
  * `<<VX_ERROR_INVALID_PARAMETERS,VX_ERROR_INVALID_PARAMETERS>>` - If any of the other parameters are
    incorrect.
  * `<<VX_ERROR_INVALID_MODULE,VX_ERROR_INVALID_MODULE>>` - The module does not contain the
    entry point.
...
*Return Values*
  ...
  * `<<VX_ERROR_NOT_SUPPORTED,VX_ERROR_NOT_SUPPORTED>>` - If the attribute is not supported on this
    implementation.
  * `<<VX_ERROR_INVALID_TYPE,VX_ERROR_INVALID_TYPE>>` - If the attribute type did not match known meta
    format type.
...
The meta data attributes that can be queried are identified by this list `[*REQ-1948*]`[[REQ-1948]]:
  ...
  * `<<vx_threshold>>` : `<<VX_THRESHOLD_TYPE,VX_THRESHOLD_TYPE>>`,
    `<<VX_THRESHOLD_INPUT_FORMAT,VX_THRESHOLD_INPUT_FORMAT>>`, `<<VX_THRESHOLD_OUTPUT_FORMAT,VX_THRESHOLD_OUTPUT_FORMAT>>`
...
.Note
====
For `<<vx_image>>`, a specific attribute can be used to query the valid region
evolution.
...
// tag::refpage:vxRegisterKernelLibrary[]
// [desc='',type='protos']
==== vxRegisterKernelLibrary

// Brief text:
Registers a module with kernels in a context `[*REQ-1996*]`[[REQ-1996]].

include::api/protos/vxRegisterKernelLibrary.txt[]


// Detailed text:
This function registers the appropriate _publish_ and _unpublish_ functions
with the module name if the _module_ is not a dynamic library, so
`<<vxLoadKernels,vxLoadKernels>>` and `<<vxUnloadKernels,vxUnloadKernels>>`
can be called.

*Parameters*

  * `[in]` _context_ - The reference to the context each kernel in the module will be added to `[*REQ-1997*]`[[REQ-1997]].
  * `[in]` _module_ - The short name of the module to register `[*REQ-1998*]`[[REQ-1998]].
  * `[in]` _publish_ - The entry function for loading the module called by `<<vxLoadKernels,vxLoadKernels>>` `[*REQ-1999*]`[[REQ-1999]].
  * `[in]` _unpublish_ - The entry function for unloading the module called by `<<vxUnloadKernels,vxUnloadKernels>>` `[*REQ-2000*]`[[REQ-2000]].

*Returns:* A `<<vx_status_e>>` enumeration `[*REQ-2001*]`[[REQ-2001]].

*Return Values*

  * `<<VX_SUCCESS,VX_SUCCESS>>` - No errors; any other value indicates failure `[*REQ-2002*]`[[REQ-2002]].
  * `<<VX_ERROR_INVALID_REFERENCE,VX_ERROR_INVALID_REFERENCE>>` - context is not a valid
    `<<vx_context>>` reference.
  * `<<VX_ERROR_INVALID_PARAMETERS,VX_ERROR_INVALID_PARAMETERS>>` - If any of the other parameters are
    incorrect.

*See also:* `<<vxLoadKernels,vxLoadKernels>>`, `<<vxUnloadKernels,vxUnloadKernels>>`
// end::refpage:vxRegisterKernelLibrary[]
...
Pattern Analysis and Machine Intelligence, 32:105-119, October 2010.

[[discontinued_requirement_ids]]
= List of Discontinued Requirement Identifiers

[options="header"]
|====
| Requirement Identifier     | OpenVX Specification Revision
| `[*REQ-0043*]`[[REQ-0043]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0148*]`[[REQ-0148]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0159*]`[[REQ-0159]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0165*]`[[REQ-0165]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0185*]`[[REQ-0185]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0249*]`[[REQ-0249]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0260*]`[[REQ-0260]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0275*]`[[REQ-0275]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0369*]`[[REQ-0369]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0382*]`[[REQ-0382]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0398*]`[[REQ-0398]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0496*]`[[REQ-0496]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-0505*]`[[REQ-0505]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-1092*]`[[REQ-1092]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-1093*]`[[REQ-1093]] | Version 1.3, Thu, 08 Aug 2019 20:26:04 +0000
| `[*REQ-1153*]`[[REQ-1153]] | Version 1.3.1, Wed, 02 Feb 2022 05:48:32 +0000
| `[*REQ-1181*]`[[REQ-1181]] | Version 1.3.1, Wed, 02 Feb 2022 05:48:32 +0000
|====

Refer to the
https://www.khronos.org/registry/OpenVX[Khronos OpenVX Registry]
for the above specification revisions.
