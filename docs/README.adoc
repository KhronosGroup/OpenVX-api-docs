...
.Note
====
This README describes the OpenVX specification build process.
The most useful parts are the <<intro, Introduction>>,
<<building, Building The Spec>>, and notes on installing
...
Dependencies>> below), go to `...path-to-git-repo/api-docs/docs` .
...
targets, or they can individually be found as follows:
  ...
  * API spec:
  ** `html` - Single-file HTML5 in `$(OUTDIR)/$(SPECDIRNAME)/$(SPECDIRNAME).html`
  ** `pdf` - PDF in `$(OUTDIR)/$(SPECDIRNAME).pdf`
...
`OpenVX_Specification.adoc` while each extension is in `vx_extension_name.adoc`. Build an
...
may want to edit `OpenVX_Specification.adoc` to just include your test code.
...
//  description of the API in `vx.xml` :
//
//    * `checkinc`
//    * `checklinks`
...
//  against the canonical description of the API in `vx.xml`.
...
----
See the +++<<vxCreateContext>>+++ command for a generated API anchor.
----
...
====
  ...
  * Most of the tools on Bash for Windows are quite happy with Windows line
    ...
    The file `.gitattributes` at the top of the repository
...
Use `% python config/check-missing-apis.py` to check for missing functions in header files against adoc files.
Use `% python config/check-missing-apis.py -v` to get additional verbose information.
