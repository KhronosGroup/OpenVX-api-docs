...
$($(_MODULE)_HTML): $($(_MODULE)_DOXYFILE_MOD)
	...
	$(Q)$(COPY) $($(_MODULE)_DOXYFILE_MOD) $($(_MODULE)_DOXYFILE_MOD)_html
	$(PRINT) INPUT += docs/doxygen/copyright.md >> $($(_MODULE)_DOXYFILE_MOD)_html
	$(PRINT) GENERATE_HTML = YES >> $($(_MODULE)_DOXYFILE_MOD)_html
	$(PRINT) GENERATE_LATEX = NO >> $($(_MODULE)_DOXYFILE_MOD)_html
	$(Q)$(DOXYGEN) $($(_MODULE)_DOXYFILE_MOD)_html

$($(_MODULE)_TEX): $($(_MODULE)_DOXYFILE_MOD)
	$(Q)$(CLEANDIR) $(TARGET_DOC)/$(_MODULE)/latex
	$(Q)$(COPY) $($(_MODULE)_DOXYFILE_MOD) $($(_MODULE)_DOXYFILE_MOD)_latex
	$(PRINT) GENERATE_HTML = NO >> $($(_MODULE)_DOXYFILE_MOD)_latex
	$(PRINT) GENERATE_LATEX = YES >> $($(_MODULE)_DOXYFILE_MOD)_latex
	$(Q)$(DOXYGEN) $($(_MODULE)_DOXYFILE_MOD)_latex

$($(_MODULE)_PDF): $($(_MODULE)_TEX)
	-$(Q)cp -r docs/images/* $(TARGET_DOC)/$(_MODULE)/latex/; \
	 cd $(TARGET_DOC)/$(_MODULE)/latex/; \
	 sed -i 's/Topic Documentation/API Documentation/g' refman.tex; \
	 sed -i -E 's|\multirow\{([0-9])\}\{\*\}|\multirow\{\1\}\{\\linewidth\}|g' *.tex; \
