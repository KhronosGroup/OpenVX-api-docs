 ...
 */
// tag::add_tiling_function[]
void add_image_tiling(void * parameters[],
                      void * tile_memory,
 ...
 * \ingroup group_tiling
 */
// tag::add_fast_tiling_function[]
void add_image_tiling_fast(void * parameters[],
                           void * tile_memory,
                           vx_size tile_memory_size)
{
   vx_uint32 i,j,ii,jj;
      ...
      for (jj = 0u; jj < 16u; ++jj)
      {
         for (i = 0u; i < vxTileWidth(out,0); i+=16)
         {
            for (ii = 0u; ii < 16u; ++ii) {
               /* this math covers a 1x1 block and has no neighborhood */
               vx_uint16 pixel =
                   vxImagePixel(vx_uint8, in0, 0, i+ii, j+jj, 0, 0) +
 ...
 * \ingroup group_tiling
 */
// tag::add_flexible_tiling_function[]
...
// end::add_flexible_tiling_function[]
