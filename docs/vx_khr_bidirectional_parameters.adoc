...
c.f. `[REQ-1892]` of OpenVX V1.3.2 The third parameter (direction) to the API vxAddParameterToKernel(kernel, index, direction, data_type, state) can be in general VX_BIDIRECTIONAL, VX_INPUT, or VX_OUTPUT.
...
* `[*REQ-BP04*]`[[REQ-BP04]] A virtual object may be attached to at most one bidirectional parameter of a node in a graph only if it is also attached to an output parameter of a second node and to an input parameter of a third. It may also be connected to any number of other input parameters.
  Reasoning:
  ** As it is a virtual object, it needs to be initialized by a graph node as the data is not accessible outside the graph, therefore is needs to be connected to an output
...
`[*REQ-BP12*]`[[REQ-BP12]] c.f `[REQ-0697]` of OpenVX V1.3.2:
...
Notice that only node APIs are added when compared to the original definition of the accumulate functions in OpenVX, consequently the same kernels are re-used.
When using `vxGetKernelByName` the following are strings specifying the bidirectional parameter extension kernel names:
...
enum vx_kernel_e {
    ...
    /*! \brief The weighted accumulation kernel.
