...
group of functions is correctly implemented.  In order to claim that an implementation supports an
...
is a collection of basic framework functions that can be included in other feature sets.  Informational
feature sets are groups of features that the OpenVX group identifies as being a useful subset of
...
. Vision (core vision conformance feature set for OpenVX 1.3.2, primarily based on the OpenVX 1.1 vision baseline)
...
. Enhanced Vision (additional vision functions introduced in OpenVX 1.2 and later)
...
Details of these feature sets are described below.  This document accompanies the OpenVX 1.3.2
specification.  These feature sets were originally based on OpenVX 1.1 or higher, and reference
some APIs and symbols that may be found in the main OpenVX specification at
https://www.khronos.org/registry/OpenVX/.  They also incorporate material
in the Export and Import extension (vx_khr_ix),
and the neural network extension (vx_khr_nn).
...
The Base Feature Set also requires support for User Kernels as described in the main OpenVX
...
To provide a basic set of vision processing functions.  This set of functions is primarily based on
the set of functions available in version 1.1 of the OpenVX specification, with selected later core
additions such as WeightedAverage included in the OpenVX 1.3.2 feature-set definitions. In addition to the
...
| `vxTensorAddNode` | `vxTensorConvertDepthNode` | `vxTensorMatrixMultiplyNode`
| `vxTensorMultiplyNode` | `vxTensorSubtractNode` | `vxTensorTableLookupNode`
...
`vxImportKernelFromURL` function, plus the following data object requirements.

==== Data Object Requirements

The NNEF Import Conformance Feature Set includes the following data objects in their entirety,
including all functions, macros, typedefs, and enumerations described in their respective sections
...
prior to calling the `vxImportKernelFromURL` function.  The registered user kernel must be
consistent with the custom kernel declared in the NNEF model in terms of:
...
Since this feature set is dependent on the Vision Conformance Feature Set, all the data objects from
...
The implementation-defined behaviors must be defined and documented by the implementation.
