...
The name of this extension is *vx_khr_ix*.

[[sec_usecases]]
== Use Case

  * Embedded systems using fixed graphs, to minimize the amount of code
    required.
...
== Access To Object References In The Imported Object
  ...
  * *2.7.3* Before export, objects may be named for retrieval by name using
    the existing `vxSetReferenceName()` API.
...
images created from handles, or the export operation will fail.
====
...
framework and may be initialized by the application to NULL.
...
ones is not detected and could lead to implementation-defined behavior;
