...
:regtitle: pass:q,r[^®^]
The Khronos{regtitle} OpenVX Working Group; Editors: Radhakrishna Giduthuri, Tomer Schwartz, Mostafa Hagog
...
The format of this binary will be implementation-defined.
...
*See also:* <<Extension: Deep Convolutional Networks API>>
--
  * [[VX_KERNEL_LOCAL_RESPONSE_NORMALIZATION_LAYER]] `VX_KERNEL_LOCAL_RESPONSE_NORMALIZATION_LAYER` - The
    Neural Network Extension Local Response Normalization Kernel.
+
--
// Detailed text
*See also:* <<Extension: Deep Convolutional Networks API>>
...
input and output.
[eq]#a~x~# and [eq]#a~y~# must be positive and smaller than [eq]#upscale~x~#
...
// tag::refpage:vxLocalResponseNormalizationLayer[]
// [desc='',type='protos']
==== vxLocalResponseNormalizationLayer
...
include::api/protos/vxLocalResponseNormalizationLayer.txt[]


// Detailed text:
Local Response Normalizing over local input regions.
Each input value is divided by

latexmath:[(bias + \frac{\alpha}{n} \sum_i x^2_i)^\beta]

where [eq]#n# is the number of elements to normalize across.
and the sum is taken over a rectangle region centered at that value (zero
...
*Parameters*
  ...
  * `[in]` _alpha_ - [static] Alpha parameter in the local response normalization equation.
    must be positive.
  * `[in]` _beta_ - [static] Beta parameter in the local response normalization equation.
    must be positive.
  * `[in]` _bias_ - [static] Bias parameter in the local response normalization equation.
    must be positive.
  * `[out]` _outputs_ - The output tensor data.
    Output will have the same number of dimensions as input.
...
Any possible errors preventing a successful creation should be checked using
`vxGetStatus`.
// end::refpage:vxLocalResponseNormalizationLayer[]
