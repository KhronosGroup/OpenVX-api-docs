// Copyright (c) 2017-2026 Khronos Group. This work is licensed under a
...
1. Wait for an event and process it. Because this is for any event, it includes user events, graph parameter events, etc. on all graphs. Therefore it is necessary to create a broker process to dispatch tasks for event processing, and this architecture may not always be suitable and could require an additional signaling mechanism.
...
.Note
====
Setting the timeout attribute `<<VX_GRAPH_TIMEOUT>>` does not set any limit upon the duration of graph execution, it merely prevents the above-mentioned functions from delaying more than the time given. There are no other requirements upon what the framework should do; it is up to the application to recognize the timeout and take appropriate action.
====
...
.Note
====
Setting the timeout attribute `<<VX_GRAPH_EVENT_TIMEOUT>>` does not set any limit upon the duration of graph execution, it merely prevents the above-mentioned function from delaying more than the time given. There are no other requirements upon what the framework should do; it is up to the application to recognize the timeout and take appropriate action.
====
  * [[VX_GRAPH_PIPELINE_DEPTH]]
   `<<VX_GRAPH_PIPELINE_DEPTH>>` - Set or return the graph pipeline depth; the depth used in the graph is known after graph verification. Read-write. Use a `vx_uint32` parameter. `[*REQ-PI018*]`[[REQ-PI018]]
...
The attribute is made read-write, so that an application may modify pipeline behavior. Example use cases are to save memory by restricting pipeline depth, or to effectively turn off pipelining by setting the depth to 1 for debugging purposes.
...
`[*REQ-PI023*]`[[REQ-PI023]]
  ...
  * Here, not all graph parameters need to have enqueued references for a
    ...
    However, exact behavior is implementation specific.
...
4. If a NULL reference is otherwise specified in the list, the results are implementation-defined
 ...
 * `[in]` new_references - array holding the new references, must be same length as "number_to_add". There may be an implementation-defined limit on the total number of references per graph parameter. `[*REQ-PI070*]`[[REQ-PI070]]
...
then the trigger node is implementation-defined.
...
.Note
====
Setting timeout attributes does not in any way change the occurrence of events, it merely prevents the above-mentioned functions from delaying more than the time given. There are no other requirements upon what the framework should do; it is up to the application to recognize the timeout and take appropriate action.
====
...
contexts then its behavior is not defined by OpenVX.
...
====
.Notes
If `vxReleaseContext` or `vxReleaseGraph(&graph)` is called while an application is blocked on _vxWaitGraphEvent(Graph,...)_, the behavior is not defined.

If _vxWaitGraphEvent(Graph,...)_ is called simultaneously from different threads with the same first parameter, then the behavior is not defined.
 ...
 * `VX_ERROR_INVALID_REFERENCE` The kernel was invalid or not finalized `[*REQ-PI228*]`[[REQ-PI228]]
...
`[*REQ-PI233*]`[[REQ-PI233]]
This API is deprecated in favor of `<<vxSetGraphScheduleConfig>>` and a new function `<<vxAddReferencesToGraphParameterList>>`. If it is used, it will behave as before, but any graph parameter assignments will be scrapped if `<<vxSetGraphScheduleConfig>>` is subsequently called.

=== Deprecation of `vxGetGraphParameterByIndex`
`[*REQ-PI234*]`[[REQ-PI234]]
The parameter object returned by `vxGetGraphParameterByIndex` is insufficiently specified. The assumption has been that it is the same object as originally set using `vxAddParameterToGraph`, an API that we would like to deprecate. However, the utility of returning a parameter object in this way is also questionable, and we would like to deprecate the function in favor of a more useful interface to obtain more thorough information about all the graph parameters, i.e., `<<vxGetGraphParameterConfig>>`.

== Deprecation of the parameter object
`[*REQ-PI235*]`[[REQ-PI235]]
The `vx_parameter` object may be deprecated if the above changes are made. The following APIs and definitions may be removed if backwards-compatibility is not required:
...
| vx_khr_sub_image_object_array | No requirements
