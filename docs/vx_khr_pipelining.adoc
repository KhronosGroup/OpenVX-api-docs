...
= The OpenVX^(TM)^ Graph Pipelining, Streaming, and Batch Processing Extension to OpenVX 1.1 and 1.2
:regtitle: pass:q,r[^®^]
...
The following code offers a example of the process outlined above using
`<<VX_GRAPH_SCHEDULE_MODE_QUEUE_MANUAL,VX_GRAPH_SCHEDULE_MODE_QUEUE_MANUAL>>`
scheduling mode.
...
nodes. A source node is a node which takes no input and only outputs data to
one or more data references. A sink node is a node which takes one or more
data references as input but produces no output. For such a graph, graph
...
by an application:
  ...
  ** The implementation automatically decides the re-schedule trigger
     condition.
  * Sometimes a user node may want to stop the continuous graph execution due
    to end of stream or error condition detected within its node execution. In
    this case the user node should return an error status. When a error status
    is returned by the user node, the continuous graph execution is stopped.
  * Application can use `vxWaitGraph` to wait for streaming graph
    execution to stop on its own.
  * Alternatively, user application can explicitly stop the streaming mode of
    execution using `<<vxStopGraphStreaming>>`.
...
When graph schedule mode is `<<VX_GRAPH_SCHEDULE_MODE_QUEUE_AUTO,VX_GRAPH_SCHEDULE_MODE_QUEUE_AUTO>>`:
  ...
  * Here, not all graph parameters need to have enqueued references for a
    ...
    graph as much as possible until a enqueued reference is not available at
...
// Detailed text:
In streaming mode of graph execution, once a application starts graph
...
After streaming mode of a graph has been started, the following APIs should
_*not* be_ used on that graph by an application: `vxScheduleGraph`,
`vxWaitScheduleGraphDone`, and `vxIsScheduleGraphAllowed`.
...
// Detailed text:
  * _user_event_id_ -   user event ID associated with this event
...
// Detailed text:
  ...
  * _event_info_ -  parameter structure associated with a event. Depends on _type_ of the event
...
If `vxReleaseContext` is called while a application is blocked on
...
*Parameters*
  ...
  * `[in]` _context_ - OpenVX context
  * `[in]` _user_event_id_ - User defined event ID
  * `[in]` _user_event_parameter_ - User defined event parameter. NOT used
    by implementation. Returned to user as part
    `<<vx_event_t>>.user_event_parameter` field
