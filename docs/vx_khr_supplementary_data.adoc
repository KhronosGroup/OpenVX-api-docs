// Copyright (c) 2023-2026 Khronos Group. This work is licensed under a
...
/* You could optimize stack usage if there was no reason to have the entire my_user_data array: */
...
| vxCreateImageFromROI | vx_image | vx_image | OpenVX 1.3.2
| vxCreateImageFromChannel | vx_image | vx_image | OpenVX 1.3.2
| [[ArrayFromTensor]]vxCreateImageObjectArrayFromTensor | vx_tensor | vx_image | OpenVX 1.3.2
| vxCreateTensorFromView | vx_tensor | vx_tensor | OpenVX 1.3.2
| vxCreateObjectArrayFromROI | vx_image | vx_image | [[sub_image]]vx_khr_sub_image_object_array
| vxCreateObjectArrayFromChannel | vx_image | vx_image | vx_khr_sub_image_object_array
...
See also `<<vxExtend, vxExtendSupplementaryUserDataObject>>`, which is a generalization of this function.
...
`[*REQ-SD04*]`[[REQ-SD04]]
If no supplementary data is associated with the *destination* object, and the *destination* object is not associated with any verified graph, then a new supplementary data object shall be created using the *source* as an exemplar, and its data shall be initialized from the *source* object.
...
|====
| *VX_SUCCESS*                 | The supplementary data of *destination* is successfully initialized
...
`[*REQ-SD17*]`[[REQ-SD17]]
If the function is called from within a kernel processing function and *ref* is passed as an input parameter to the kernel function, the reference retrieved for the supplementary data shall be a <<read_only,read-only>> vx_user_data_object for the duration of the kernel processing function.
...
The reason for including this function is to minimize the number of separate buffer map operations required when copying and extending supplementary data.
