...
:leveloffset: 1
= OpenVX 1.3.2: SWAP and MOVE: New kernels with bidirectional parameters
[[sec_purpose]]
== Purpose
...
Users should avoid using MOVE or COPY where objects are in different memory areas (implementation-defined).

== Kernel Names

`[*REQ-SWP19*]`[[REQ-SWP19]]

When using `vxGetKernelByName` the following are strings specifying the swap and move kernel extension kernel names:
