...
# defaultValue - return specified value, a default value if None is
# specified, and optionally generate a warning
#
# specified - the value
# default - value to return if specified is None
# warning - string warning to raise if specified is None
def defaultValue(specified, default, warning = None):
    if specified:
        return specified
    else:
        if warning != None:
            write('*** WARNING:', warning, file=sys.stderr)
        return default
...
class EnumInfo(BaseInfo):
    ...
    def __init__(self, elem):
        BaseInfo.__init__(self, elem)
        self.type = defaultValue(elem.get('type'), '')
...
class FeatureInfo(BaseInfo):
    ...
    def __init__(self, elem):
        ...
        if (elem.tag == 'feature'):
            ...
            self.versionNumber = defaultValue(elem.get('number'),
                None,
                'No version number specified for feature ' + self.name)
            self.number = "0"
            self.supported = None
        else:
            ...
            # For Vulkan, use warning text
            #   'No extension number specified for extension ' + self.name)
            # It requires (badly needs) extension numbers, OpenVX does not.
            self.number = defaultValue(elem.get('number'),
                0,
                None)
    ...
    def parseTree(self):
        ...
        for type in self.reg.findall('types/type'):
            ...
            if (type.get('name') == None):
                elem = type.find('name')
                if elem != None:
                    type.attrib['name'] = elem.text
                else:
                    self.gen.logMsg('warn', 'parseTree: <type> element has no name attribute or element:',
                                    type.text)
        ...
        for feature in self.extensions:
            ...
            for elem in feature.findall('require'):
              ...
              # This part handles resolving aliases, by figuring out what type they are, and then appropriately putting in an alias.
              # For commands, a duplicate command is introduced with the aliasing name
              # For enum values, a #define aliases the original enum value
              # For types, a typedef is introduced

              # First, loop through the aliases, find their base, figure out what they are, and construct a string representing the new required xml element.
              elementReplacements = {}
              for alias in elem.findall('alias'):
                aliasName = alias.get('name')
                aliasValue = alias.get('value')

                # Only need to find one matching element - if there are duplicates they're either legit or should be caught elsewhere in the code.
                base = self.reg.find('.//*[@name="' + aliasValue + '"]')

                # Construct a new string for the alias
                newString = ''
                if base.tag == 'enum':
                  newString = '<enum value="' + aliasValue + '" name="' + aliasName + '"/>'
                elif base.tag == 'type':
                  # FIXME? Should category really be "define" here? Should we have a new "typedef" category?
                  newString = '<type name="' + aliasName + '" category="define">typedef <type>' + aliasValue + '</type> <name>' + aliasName + '</name>;</type>'
                elif base.tag == 'command':
                  newString = etree.tostring(base, encoding="unicode").replace(aliasValue, aliasName)
                else:
                  raise UserWarning('Base of the alias ' + aliasName + ', ' + aliasValue + ' has an unhandled element tag')
                elementReplacements[alias] = newString

              # Go through the list of replacements generated in the prior loop,
              # replace all the alias elements with resolved elements in the element tree,
              # then add them to the registry.
              # Will need to sort on the alias name, not the replacement name,
              # if we have multiple aliases of the same name.
              for (alias,replacement) in sorted(elementReplacements.items(),
                                    key = lambda x : x[0].get('name').lower()):
                elem.remove(alias)
                elem.append(etree.fromstring(replacement))
                newElement = list(elem)[-1]

                if newElement.tag == 'enum':
                  info = EnumInfo(newElement)
                  self.addElementInfo(newElement, info, 'enum', self.enumdict)
                elif newElement.tag == 'type':
                  info = TypeInfo(newElement)
                  self.addElementInfo(newElement, info, 'type', self.typedict)
                elif newElement.tag == 'command':
                  info = CmdInfo(newElement)
                  self.addElementInfo(newElement, info, 'command', self.cmddict)
        ...
        if (ftype == 'type'):
            ...
            if (f.elem.get('category') == 'enum'):
                ...
                if (group == None):
                    ...
                    genProc = None
                    self.gen.logMsg('warn', '*** NO MATCHING ENUM GROUP FOUND!!!')
                else:
